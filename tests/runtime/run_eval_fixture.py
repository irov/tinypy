...
def main() -> int:
    ...
    subprocess.run([str(arguments.runner), "--eval-any", str(artifact)], check=True)
    source = root / "tests/runtime/fixtures/comparison_runtime.py"
    artifact = arguments.build_dir / "comparison_runtime.marshal"
    subprocess.run([str(arguments.compiler), str(source), str(artifact), "tests/runtime/fixtures/comparison_runtime.py", "exec", "0"], check=True)
    subprocess.run([str(arguments.runner), "--eval-any", str(artifact)], check=True)
    source = root / "tests/runtime/fixtures/control_flow_runtime.py"
    artifact = arguments.build_dir / "control_flow_runtime.marshal"
    subprocess.run([str(arguments.compiler), str(source), str(artifact), "tests/runtime/fixtures/control_flow_runtime.py", "exec", "0"], check=True)
    subprocess.run([str(arguments.runner), "--eval-any", str(artifact)], check=True)
    source = root / "tests/runtime/fixtures/legacy_slice_runtime.py"
    artifact = arguments.build_dir / "legacy_slice_runtime.marshal"
    subprocess.run([str(arguments.compiler), str(source), str(artifact), "tests/runtime/fixtures/legacy_slice_runtime.py", "exec", "0"], check=True)
    subprocess.run([str(arguments.runner), "--eval-any", str(artifact)], check=True)
