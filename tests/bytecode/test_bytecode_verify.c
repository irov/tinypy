...
static tinypy_bytecode_verify_status_e __test_verify(
    const uint8_t *code,
    size_t code_size,
...
static size_t __test_emit_instruction(
    uint8_t *code,
    size_t offset,
    uint8_t opcode,
    uint16_t argument)
...
static void __test_patch_argument(
    uint8_t *code,
...
static int __test_all_defined_opcodes_reach_effect_model(void)
{
    ...
    for (opcode_value = 0U;
        ...
        if (opcode == TINYPY_OP_POP_BLOCK ||
            opcode == TINYPY_OP_BREAK_LOOP ||
            opcode == TINYPY_OP_CONTINUE_LOOP) {
            prefix_setup = offset;
            offset = __test_emit_instruction(
                code,
                offset,
...
int main(void)
{
    if (__test_simple_and_terminators() != 0) return 1;
    if (__test_valid_loop() != 0) return 1;
    if (__test_valid_try_except() != 0) return 1;
    if (__test_valid_try_finally_and_return() != 0) return 1;
    if (__test_valid_with_and_generator() != 0) return 1;
    if (__test_nested_pending_reasons() != 0) return 1;
    if (__test_variable_stack_effects() != 0) return 1;
    if (__test_all_defined_opcodes_reach_effect_model() != 0) return 1;
    if (__test_decode_and_opcode_errors() != 0) return 1;
    if (__test_operand_bounds() != 0) return 1;
    if (__test_invalid_special_operands() != 0) return 1;
    if (__test_jump_validation() != 0) return 1;
    if (__test_control_flow_errors() != 0) return 1;
    if (__test_declared_stack_limits_and_block_limits() != 0) return 1;
    if (__test_scratch_contract() != 0) return 1;
    if (__test_status_names() != 0) return 1;
    return 0;
}
