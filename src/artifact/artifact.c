...
static int __tinypy_artifact_metadata_valid(
    const tinypy_artifact_metadata_t *metadata)
{
    ...
    has_source_map = (metadata->feature_flags &
        (uint32_t)TINYPY_ARTIFACT_FEATURE_SOURCE_MAP) != 0U;
    ...
    return metadata->abi_version == TINYPY_ARTIFACT_ABI_VERSION &&
        metadata->struct_size >= sizeof(tinypy_artifact_metadata_t) &&
        metadata->reserved == 0U &&
        metadata->bytecode_magic == TINYPY_CPYTHON_27_MAGIC &&
        metadata->runtime_abi != 0U &&
        metadata->compiler_abi != 0U &&
        metadata->meta_abi != 0U &&
        metadata->preprocessor_abi != 0U &&
        metadata->optimize_level <= 2U &&
        metadata->payload_kind == (uint8_t)TINYPY_ARTIFACT_PAYLOAD_MARSHAL_V2 &&
        (metadata->feature_flags & ~TINYPY_ARTIFACT_ALL_FEATURES) == 0U &&
        has_source_map == (source_map_is_zero == 0);
}

tinypy_artifact_status_e tinypy_artifact_encode(
    const tinypy_artifact_metadata_t *metadata,
    const void *payload,
    size_t payload_size,
    void *output,
...
tinypy_artifact_status_e tinypy_artifact_decode(
    const void *artifact,
    size_t artifact_size,
    size_t max_payload_size,
    tinypy_artifact_view_t *out_view)
{
    ...
    if (__tinypy_artifact_read_u16_le(
            bytes + TINYPY_ARTIFACT_OFFSET_FORMAT_VERSION) !=
            TINYPY_ARTIFACT_FORMAT_VERSION ||
        __tinypy_artifact_read_u16_le(bytes + TINYPY_ARTIFACT_OFFSET_HEADER_SIZE) !=
            TINYPY_ARTIFACT_HEADER_SIZE) {
        return TINYPY_ARTIFACT_UNSUPPORTED_FORMAT;
    }
    if (bytes[TINYPY_ARTIFACT_OFFSET_PYTHON_MAJOR] != 2U ||
        bytes[TINYPY_ARTIFACT_OFFSET_PYTHON_MAJOR + 1U] != 7U ||
        bytes[TINYPY_ARTIFACT_OFFSET_PYTHON_MAJOR + 2U] != 18U ||
        bytes[TINYPY_ARTIFACT_OFFSET_MARSHAL_VERSION] != 2U) {
    ...
    if (bytes[TINYPY_ARTIFACT_OFFSET_PAYLOAD_KIND] !=
            (uint8_t)TINYPY_ARTIFACT_PAYLOAD_MARSHAL_V2 ||
        bytes[TINYPY_ARTIFACT_OFFSET_WIRE_ENCODING] !=
            TINYPY_ARTIFACT_WIRE_ENCODING_LE) {
        return TINYPY_ARTIFACT_UNSUPPORTED_PAYLOAD;
    }
    if (bytes[TINYPY_ARTIFACT_OFFSET_RESERVED_BYTE] != 0U ||
        __tinypy_artifact_read_u32_le(
            bytes + TINYPY_ARTIFACT_OFFSET_RESERVED_WORD) != 0U ||
        __tinypy_artifact_read_u32_le(bytes + TINYPY_ARTIFACT_OFFSET_HEADER_CRC) !=
            __tinypy_artifact_crc32(bytes, TINYPY_ARTIFACT_OFFSET_HEADER_CRC)) {
...
static int __tinypy_artifact_field_equal(
    uint32_t flags,
    uint32_t flag,
    uint32_t actual,
    uint32_t expected)
...
tinypy_artifact_status_e tinypy_artifact_check_profile(
    const tinypy_artifact_view_t *view,
    const tinypy_artifact_expectation_t *expectation)
{
    uint32_t flags;
    ...
    if (view->abi_version != TINYPY_ARTIFACT_ABI_VERSION ||
        view->struct_size < sizeof(tinypy_artifact_view_t) ||
        view->metadata.abi_version != TINYPY_ARTIFACT_ABI_VERSION ||
        view->metadata.struct_size < sizeof(tinypy_artifact_metadata_t) ||
        expectation->abi_version != TINYPY_ARTIFACT_ABI_VERSION ||
        expectation->struct_size < sizeof(tinypy_artifact_expectation_t) ||
        expectation->expected.abi_version !=
            TINYPY_ARTIFACT_ABI_VERSION ||
        expectation->expected.struct_size <
            sizeof(tinypy_artifact_metadata_t)) {
        return TINYPY_ARTIFACT_ABI_MISMATCH;
    }
    flags = expectation->check_flags;
    if (expectation->reserved != 0U ||
        (flags & ~TINYPY_ARTIFACT_ALL_CHECKS) != 0U) {
    ...
    if (!__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_BYTECODE_MAGIC,
            actual->bytecode_magic, expected->bytecode_magic) ||
        !__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_RUNTIME_ABI,
            actual->runtime_abi, expected->runtime_abi) ||
        !__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_COMPILER_ABI,
            actual->compiler_abi, expected->compiler_abi) ||
        !__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_META_ABI,
            actual->meta_abi, expected->meta_abi) ||
        !__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_PREPROCESSOR_ABI,
            actual->preprocessor_abi, expected->preprocessor_abi) ||
        !__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_FEATURE_FLAGS,
            actual->feature_flags, expected->feature_flags) ||
        !__tinypy_artifact_field_equal(flags, TINYPY_ARTIFACT_CHECK_FUTURE_FLAGS,
            actual->future_flags, expected->future_flags) ||
        ((flags & TINYPY_ARTIFACT_CHECK_OPTIMIZE_LEVEL) != 0U &&
            actual->optimize_level != expected->optimize_level) ||
        ((flags & TINYPY_ARTIFACT_CHECK_PAYLOAD_KIND) != 0U &&
            actual->payload_kind != expected->payload_kind) ||
        ((flags & TINYPY_ARTIFACT_CHECK_CONSTANT_PROFILE) != 0U &&
            memcmp(actual->constant_profile_digest,
                expected->constant_profile_digest, 32U) != 0) ||
        ((flags & TINYPY_ARTIFACT_CHECK_SOURCE_HASH) != 0U &&
            memcmp(actual->source_hash, expected->source_hash, 32U) != 0) ||
        ((flags & TINYPY_ARTIFACT_CHECK_SOURCE_MAP_HASH) != 0U &&
            memcmp(actual->source_map_hash,
...
{
    switch (status) {
    case TINYPY_ARTIFACT_OK: return "ok";
    case TINYPY_ARTIFACT_INVALID_ARGUMENT: return "invalid argument";
    case TINYPY_ARTIFACT_ABI_MISMATCH: return "ABI mismatch";
    case TINYPY_ARTIFACT_BUFFER_TOO_SMALL: return "buffer or limit too small";
    case TINYPY_ARTIFACT_SIZE_OVERFLOW: return "size overflow";
    case TINYPY_ARTIFACT_TRUNCATED: return "truncated artifact";
    case TINYPY_ARTIFACT_BAD_MAGIC: return "bad artifact magic";
    case TINYPY_ARTIFACT_UNSUPPORTED_FORMAT: return "unsupported artifact format";
    case TINYPY_ARTIFACT_UNSUPPORTED_LANGUAGE: return "unsupported language ABI";
    case TINYPY_ARTIFACT_UNSUPPORTED_PAYLOAD: return "unsupported payload";
    case TINYPY_ARTIFACT_CORRUPT_HEADER: return "corrupt artifact header";
    case TINYPY_ARTIFACT_CORRUPT_PAYLOAD: return "corrupt artifact payload";
    case TINYPY_ARTIFACT_TRAILING_DATA: return "trailing artifact data";
    case TINYPY_ARTIFACT_PAYLOAD_LIMIT: return "artifact payload limit exceeded";
    case TINYPY_ARTIFACT_PROFILE_MISMATCH: return "artifact profile mismatch";
    default: return "unknown artifact status";
    }
}
