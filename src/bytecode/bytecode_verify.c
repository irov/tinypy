...
static int __tinypy_verify_add_size(
    size_t left,
    size_t right,
    size_t *out_value)
{
...
static int __tinypy_verify_multiply_size(
    size_t left,
    size_t right,
    size_t *out_value)
{
...
static int __tinypy_verify_align_size(
    size_t value,
    size_t *out_value)
{
    size_t remainder;
...
static void __tinypy_verify_clear_result(
    tinypy_bytecode_verify_result_t *result)
{
    size_t index;
...
static tinypy_bytecode_verify_status_e __tinypy_verify_fail(
...
tinypy_bytecode_verify_status_e tinypy_bytecode_verify_scratch_size(
    size_t bytecode_size,
    size_t *out_size)
{
    size_t state_count;
    ...
    *out_size = 0U;
    ...
    total = bytecode_size;
    if (!__tinypy_verify_align_size(total, &total) ||
        !__tinypy_verify_add_size(total, state_heads_size, &total)) {
        return TINYPY_BYTECODE_VERIFY_SIZE_OVERFLOW;
    }
    if (!__tinypy_verify_align_size(total, &total) ||
        !__tinypy_verify_add_size(total, states_size, &total) ||
        !__tinypy_verify_align_size(total, &total) ||
        !__tinypy_verify_add_size(total, worklist_size, &total) ||
        !__tinypy_verify_align_size(total, &total) ||
        !__tinypy_verify_add_size(total, blocks_size, &total) ||
        !__tinypy_verify_align_size(total, &total) ||
        !__tinypy_verify_add_size(total, markers_size, &total)) {
        return TINYPY_BYTECODE_VERIFY_SIZE_OVERFLOW;
    }
...
static void __tinypy_verify_apply_default_limits(
    const tinypy_bytecode_verify_limits_t *source,
    tinypy_bytecode_verify_limits_t *destination)
{
    if (source == NULL) {
...
static void __tinypy_verify_bind_scratch(
    tinypy_verify_context_t *context,
    void *scratch)
{
    uint8_t *base = (uint8_t *)scratch;
    size_t offset = context->bytecode_size;
    size_t state_count = context->bytecode_size *
        (size_t)TINYPY_BYTECODE_VERIFY_CONTEXTS_PER_BYTE;
...
static int __tinypy_verify_argument_to_size(
    uint64_t argument,
    size_t *out_value)
{
    assert(out_value != NULL);
...
static tinypy_bytecode_verify_status_e __tinypy_verify_operand(
    tinypy_verify_context_t *context,
    const tinypy_decoded_instruction_t *instruction)
{
    ...
    if ((categories & TINYPY_OPCODE_CATEGORY_CONST) != 0U &&
        argument >= (uint64_t)context->metadata->const_count) {
        return __tinypy_verify_fail(
            context,
    ...
    if ((categories & TINYPY_OPCODE_CATEGORY_NAME) != 0U &&
        argument >= (uint64_t)context->metadata->name_count) {
        return __tinypy_verify_fail(
            context,
            TINYPY_BYTECODE_VERIFY_OPERAND_OUT_OF_RANGE,
            instruction->offset,
    ...
    if ((categories & TINYPY_OPCODE_CATEGORY_LOCAL) != 0U &&
        argument >= (uint64_t)context->metadata->varname_count) {
        return __tinypy_verify_fail(
            context,
            TINYPY_BYTECODE_VERIFY_OPERAND_OUT_OF_RANGE,
            instruction->offset,
...
static tinypy_bytecode_verify_status_e __tinypy_verify_first_pass(
    tinypy_verify_context_t *context)
{
    size_t offset = 0U;
    ...
    while (offset != context->bytecode_size) {
        ...
        if (context->limits.max_instruction_count != 0U &&
            context->result->instruction_count >
...
static int __tinypy_verify_jump_target(
    const tinypy_decoded_instruction_t *instruction,
    size_t *out_target)
{
    size_t argument;
...
static tinypy_bytecode_verify_status_e __tinypy_verify_jumps(
    tinypy_verify_context_t *context)
{
    size_t offset = 0U;
    ...
    while (offset != context->bytecode_size) {
        ...
        if ((categories &
                (TINYPY_OPCODE_CATEGORY_JREL | TINYPY_OPCODE_CATEGORY_JABS)) != 0U) {
            size_t target;

            if (!__tinypy_verify_jump_target(&instruction, &target) ||
                target >= context->bytecode_size) {
                return __tinypy_verify_fail(
                    context,
...
static uint8_t __tinypy_verify_opcode_at(
    const tinypy_verify_context_t *context,
    size_t offset)
{
    tinypy_decoded_instruction_t instruction;

    if (offset >= context->bytecode_size ||
        tinypy_opcode_decode(
            context->bytecode,
            context->bytecode_size,
            offset,
...
static int __tinypy_verify_blocks_equal(
    const tinypy_verify_context_t *context,
    size_t left,
    size_t right)
{
    while (left != TINYPY_VERIFY_NO_BLOCK && right != TINYPY_VERIFY_NO_BLOCK) {
        ...
        if (left_block->handler != right_block->handler ||
            left_block->level != right_block->level ||
            left_block->depth != right_block->depth ||
            left_block->type != right_block->type) {
            return 0;
        }
...
static size_t __tinypy_verify_reason_item_count(
    tinypy_verify_reason_e reason)
{
    switch (reason) {
...
static int __tinypy_verify_markers_equal(
    const tinypy_verify_context_t *context,
    size_t left,
    size_t right)
{
    while (left != TINYPY_VERIFY_NO_MARKER &&
        right != TINYPY_VERIFY_NO_MARKER) {
        ...
        if (left_marker->resume_depth != right_marker->resume_depth ||
            left_marker->continue_target != right_marker->continue_target ||
            left_marker->reason != right_marker->reason ||
            left_marker->needs_with_cleanup !=
    ...
    return left == TINYPY_VERIFY_NO_MARKER &&
        right == TINYPY_VERIFY_NO_MARKER;
}

static int __tinypy_verify_marker_expected_depth(
    const tinypy_verify_marker_t *marker,
    size_t *out_depth)
{
    return __tinypy_verify_add_size(
        marker->resume_depth,
        __tinypy_verify_reason_item_count(
            (tinypy_verify_reason_e)marker->reason),
        out_depth);
}

static tinypy_bytecode_verify_status_e __tinypy_verify_push_marker(
    tinypy_verify_context_t *context,
    const tinypy_decoded_instruction_t *instruction,
    size_t parent,
    tinypy_verify_reason_e reason,
...
static tinypy_bytecode_verify_status_e __tinypy_verify_enqueue(
    tinypy_verify_context_t *context,
    size_t offset,
    size_t stack_depth,
    size_t block_index,
    ...
    uint8_t source_opcode)
{
    ...
    while (marker_index != TINYPY_VERIFY_NO_MARKER) {
        ...
        if (marker->reason !=
            ...
            stack_depth != marker->resume_depth) {
            break;
        }
    ...
    if (context->limits.max_stack_depth != 0U &&
        stack_depth > context->limits.max_stack_depth) {
        return __tinypy_verify_fail(
            context,
    ...
    if (marker_index != TINYPY_VERIFY_NO_MARKER) {
        ...
        } else if (marker->needs_with_cleanup != 0U &&
            !__tinypy_verify_add_size(
                minimum_depth,
                1U,
                &minimum_depth)) {
            return __tinypy_verify_fail(
        ...
        if (__tinypy_verify_markers_equal(
            ...
            if (!__tinypy_verify_blocks_equal(
                ...
                return __tinypy_verify_fail(
                    ...
                    offset,
                    __tinypy_verify_opcode_at(context, offset),
                    (uint64_t)stack_depth,
                    TINYPY_OPCODE_DECODE_OK);
            }
    ...
    if (context->state_count == context->state_capacity ||
        context->worklist_tail == context->state_capacity) {
            return __tinypy_verify_fail(
                context,
                TINYPY_BYTECODE_VERIFY_LIMIT_EXCEEDED,
                source_offset,
...
static int __tinypy_verify_effect_set(
...
static int __tinypy_verify_effect_from_argument(
    uint64_t argument,
    size_t extra,
    size_t *out_value)
{
    ...
    return __tinypy_verify_argument_to_size(argument, &value) &&
        __tinypy_verify_add_size(value, extra, out_value);
}

static int __tinypy_verify_get_effect(
    uint8_t opcode,
    uint64_t argument,
    tinypy_verify_effect_t *effect)
{
    ...
    case TINYPY_OP_CALL_FUNCTION_VAR_KW:
    {
        ...
        if (opcode == TINYPY_OP_CALL_FUNCTION_VAR ||
            opcode == TINYPY_OP_CALL_FUNCTION_KW) {
            extra = 1U;
        } else if (opcode == TINYPY_OP_CALL_FUNCTION_VAR_KW) {
            extra = 2U;
        }
        if (!__tinypy_verify_multiply_size(keyword, 2U, &count) ||
            !__tinypy_verify_add_size(count, positional, &count) ||
            !__tinypy_verify_add_size(count, extra, &count) ||
            !__tinypy_verify_add_size(count, 1U, &count)) {
            return 0;
        }
        return __tinypy_verify_effect_set(effect, count, count, 1U);
    }
...
static tinypy_bytecode_verify_status_e __tinypy_verify_apply_effect(
    tinypy_verify_context_t *context,
    const tinypy_decoded_instruction_t *instruction,
    size_t stack_depth,
    const tinypy_verify_effect_t *effect,
    size_t *out_depth)
{
    size_t depth;

    if (stack_depth < effect->required ||
        stack_depth < effect->pop_count) {
        return __tinypy_verify_fail(
            context,
...
static tinypy_bytecode_verify_status_e __tinypy_verify_push_block(
    tinypy_verify_context_t *context,
    const tinypy_decoded_instruction_t *instruction,
    size_t parent,
    size_t level,
...
static tinypy_bytecode_verify_status_e __tinypy_verify_unwind_reason(
    tinypy_verify_context_t *context,
    const tinypy_decoded_instruction_t *instruction,
    size_t stack_depth,
    size_t block_index,
    ...
    size_t continue_target)
{
    while (block_index != TINYPY_VERIFY_NO_BLOCK) {
        ...
        if (block->type == TINYPY_OP_SETUP_LOOP &&
            reason == TINYPY_VERIFY_REASON_CONTINUE) {
            if (stack_depth != block->level) {
                return __tinypy_verify_fail(
                    context,
                    TINYPY_BYTECODE_VERIFY_STACK_DEPTH_MISMATCH,
        ...
        if (block->type == TINYPY_OP_SETUP_FINALLY ||
            block->type == TINYPY_OP_SETUP_WITH) {
            ...
            if (!__tinypy_verify_add_size(
                    block->level,
                    __tinypy_verify_reason_item_count(reason),
                    &handler_depth)) {
                return __tinypy_verify_fail(
                    context,
                    TINYPY_BYTECODE_VERIFY_STACK_OVERFLOW,
...
static tinypy_bytecode_verify_status_e __tinypy_verify_fallthrough(
    tinypy_verify_context_t *context,
    const tinypy_decoded_instruction_t *instruction,
    size_t stack_depth,
    size_t block_index,
...
static tinypy_bytecode_verify_status_e __tinypy_verify_process_state(
    tinypy_verify_context_t *context,
    size_t state_index)
{
    ...
    case TINYPY_OP_SETUP_WITH:
    {
        ...
        if (instruction.opcode == TINYPY_OP_SETUP_WITH) {
            if (level == 0U ||
                !__tinypy_verify_add_size(level, 3U, &depth)) {
                return __tinypy_verify_fail(
                    context,
                    TINYPY_BYTECODE_VERIFY_INVALID_FINALLY_STATE,
                    offset,
    ...
    case TINYPY_OP_WITH_CLEANUP:
    {
        ...
        if (state.marker_index != TINYPY_VERIFY_NO_MARKER) {
            ...
            if (marker->needs_with_cleanup != 0U &&
                expected != SIZE_MAX &&
                state.stack_depth == expected + 1U) {
        ...
        if (handles_pending_with == 0) {
            ...
            if (marker != NULL) {
                ...
                if (state.stack_depth < protected_depth ||
                    state.stack_depth - protected_depth < 2U) {
                    return __tinypy_verify_fail(
                        context,
                        TINYPY_BYTECODE_VERIFY_INVALID_FINALLY_STATE,
                        offset,
    ...
    case TINYPY_OP_END_FINALLY:
        ...
        }
        {
            ...
            if (marker_reason ==
                    TINYPY_VERIFY_REASON_EXCEPTION_CATCHABLE ||
                marker_reason == TINYPY_VERIFY_REASON_EXCEPTION_FINAL) {
                return TINYPY_BYTECODE_VERIFY_OK;
            }
...
static tinypy_bytecode_verify_status_e __tinypy_verify_control_flow(
    tinypy_verify_context_t *context)
{
    tinypy_bytecode_verify_status_e status;
...
tinypy_bytecode_verify_status_e tinypy_bytecode_verify(
    const uint8_t *bytecode,
    size_t bytecode_size,
    const tinypy_bytecode_metadata_t *metadata,
    const tinypy_bytecode_verify_limits_t *limits,
    void *scratch,
    size_t scratch_size,
    tinypy_bytecode_verify_result_t *out_result)
{
    tinypy_verify_context_t context;
    tinypy_bytecode_verify_status_e status;
    ...
    if (context.limits.max_bytecode_size != 0U &&
        bytecode_size > context.limits.max_bytecode_size) {
        return __tinypy_verify_fail(
            &context,
            TINYPY_BYTECODE_VERIFY_LIMIT_EXCEEDED,
            0U,
...
const char *tinypy_bytecode_verify_status_name(
    tinypy_bytecode_verify_status_e status)
{
    switch (status) {
