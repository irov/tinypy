...
tinypy_value_t *tinypy_super_new(tinypy_type_t *type, tinypy_value_t *object, tinypy_error_t **out_error)
{
    ...
    if (object != NULL) {
        if (tinypy_internal_value_kind(object) == TINYPY_VALUE_TYPE && tinypy_type_is_subtype((tinypy_type_t *)object, type) != 0) object_type = (tinypy_type_t *)object;
        else if (tinypy_type_is_subtype(object->type, type) != 0) object_type = object->type;
        else {
    ...
    if (object != NULL) tinypy_retain(object);
    if (object_type != NULL) tinypy_retain(&object_type->base.base);
...
void tinypy_internal_super_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    if (super_value->object != NULL) visit(super_value->object, user_data);
    if (super_value->object_type != NULL) visit(&super_value->object_type->base.base, user_data);
}
...
tinypy_value_t *tinypy_internal_super_get_attribute(tinypy_value_t *value, tinypy_value_t *name, tinypy_error_t **out_error)
{
    ...
    for (index = 0U; index < mro_size; index += 1U) {
        ...
        if (found_type == 0) {
            if (mro_type == super_value->type) found_type = 1;
            continue;
        }
...
tinypy_value_t *tinypy_internal_super_create(tinypy_type_t *type, tinypy_value_t *args, tinypy_value_t *kwargs, tinypy_error_t **out_error)
{
    ...
    return tinypy_super_new((tinypy_type_t *)requested_type, count == 2U ? tinypy_tuple_get(args, 1U) : NULL, out_error);
}

const tinypy_type_t *tinypy_super_type(const tinypy_value_t *super_value) { return __tinypy_internal_super_validate(super_value)->type; }
tinypy_value_t *tinypy_super_object(const tinypy_value_t *super_value) { return __tinypy_internal_super_validate(super_value)->object; }
const tinypy_type_t *tinypy_super_object_type(const tinypy_value_t *super_value) { return __tinypy_internal_super_validate(super_value)->object_type; }
