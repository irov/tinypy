...
int32_t tinypy_truth(tinypy_value_t *value, tinypy_error_t **out_error)
{
    ...
    switch (kind) {
    case TINYPY_VALUE_NONE: return INT32_C(0);
    case TINYPY_VALUE_BOOL:
    case TINYPY_VALUE_INTEGER: return TINYPY_INTEGER_VALUE(value) != 0 ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_LONG: return TINYPY_LONG_SIGN(value) != 0 ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_FLOAT: return TINYPY_FLOAT_OBJECT(value)->value != 0.0 ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_COMPLEX: return TINYPY_COMPLEX_OBJECT(value)->real != 0.0 || TINYPY_COMPLEX_OBJECT(value)->imaginary != 0.0 ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_STRING:
    case TINYPY_VALUE_UNICODE:
    case TINYPY_VALUE_TUPLE:
    case TINYPY_VALUE_LIST: return TINYPY_SIZE(value) != 0 ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_DICT: return TINYPY_DICT_OBJECT(value)->used != 0U ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_SET:
    case TINYPY_VALUE_FROZENSET: return tinypy_set_size(value) != 0U ? INT32_C(1) : INT32_C(0);
    case TINYPY_VALUE_XRANGE: return TINYPY_XRANGE_OBJECT(value)->length != 0U ? INT32_C(1) : INT32_C(0);
    default: break;
    }
    {
    ...
    if (value->type->number_slots != NULL && value->type->number_slots->nonzero != NULL) return value->type->number_slots->nonzero(value, out_error);
    if (tinypy_internal_object_has_special(value, "__nonzero__", 11U) != 0) {
        ...
        if (result == NULL) return INT32_C(-1);
        if (tinypy_internal_value_kind(result) != TINYPY_VALUE_BOOL && tinypy_internal_value_kind(result) != TINYPY_VALUE_INTEGER) {
    ...
    if (tinypy_internal_object_has_special(value, "__len__", 7U) != 0) {
        ...
        if (tinypy_internal_value_kind(result) == TINYPY_VALUE_BOOL || tinypy_internal_value_kind(result) == TINYPY_VALUE_INTEGER) {
            ...
            truth = TINYPY_INTEGER_VALUE(result) != 0 ? INT32_C(1) : INT32_C(0);
        } else if (tinypy_internal_value_kind(result) == TINYPY_VALUE_LONG) {
...
static int32_t __tinypy_comparison_type_name_order(const tinypy_type_t *left, const tinypy_type_t *right)
{
    ...
    if (left->name_size != right->name_size) return left->name_size < right->name_size ? -1 : 1;
    ...
    *out_unordered = 0;
    if (__tinypy_comparison_is_numeric(left_kind) != 0 && __tinypy_comparison_is_numeric(right_kind) != 0) {
        if (left_kind == TINYPY_VALUE_COMPLEX || right_kind == TINYPY_VALUE_COMPLEX) {
            tinypy_internal_make_vm_error(tinypy_internal_value_vm(left), TINYPY_ERROR_TYPE, "complex numbers have no ordering relation", out_error);
            return 0;
        }
...
#endif
        for (index = 0U; index < common_size; index += 1U) {
            tinypy_value_t *left_item = left_kind == TINYPY_VALUE_TUPLE ? tinypy_tuple_get(left, index) : tinypy_list_get(left, index);
            tinypy_value_t *right_item = right_kind == TINYPY_VALUE_TUPLE ? tinypy_tuple_get(right, index) : tinypy_list_get(right, index);
    ...
    if ((left_kind == TINYPY_VALUE_SET || left_kind == TINYPY_VALUE_FROZENSET) && (right_kind == TINYPY_VALUE_SET || right_kind == TINYPY_VALUE_FROZENSET)) {
        ...
        if (tinypy_set_size(left) < tinypy_set_size(right)) {
            tinypy_dict_object_t *dict = TINYPY_DICT_OBJECT(TINYPY_SET_OBJECT(left)->dict);
            size_t index;
            ...
            for (index = 0U; index <= dict->mask; index += 1U) {
                ...
                if (entry->state == TINYPY_DICT_ENTRY_ACTIVE && tinypy_dict_contains(TINYPY_SET_OBJECT(right)->dict, entry->key) == 0) {
                    left_subset = 0;
                    break;
                }
            }
            ...
            tinypy_dict_object_t *dict = TINYPY_DICT_OBJECT(TINYPY_SET_OBJECT(right)->dict);
            size_t index;
            ...
            for (index = 0U; index <= dict->mask; index += 1U) {
                ...
                if (entry->state == TINYPY_DICT_ENTRY_ACTIVE && tinypy_dict_contains(TINYPY_SET_OBJECT(left)->dict, entry->key) == 0) {
                    right_subset = 0;
                    break;
                }
            }
...
static int32_t __tinypy_comparison_text_contains(tinypy_value_t *container, tinypy_value_t *item, tinypy_error_t **out_error)
{
    ...
    if ((tinypy_internal_value_kind(item) != TINYPY_VALUE_STRING && tinypy_internal_value_kind(item) != TINYPY_VALUE_UNICODE)) {
        tinypy_internal_make_vm_error(tinypy_internal_value_vm(container), TINYPY_ERROR_TYPE, "string containment requires a string operand", out_error);
        return -1;
    }
    container_bytes = tinypy_internal_text_bytes(container);
    item_bytes = tinypy_internal_text_bytes(item);
    container_size = tinypy_internal_text_byte_size(container);
    item_size = tinypy_internal_text_byte_size(item);
    if (item_size == 0U) return 1;
    if (item_size > container_size) return 0;
    for (index = 0U; index <= container_size - item_size; index += 1U) {
        if (memcmp(container_bytes + index, item_bytes, item_size) == 0) return 1;
    }
    return 0;
}
...
int32_t tinypy_contains(tinypy_value_t *container, tinypy_value_t *item, tinypy_error_t **out_error)
{
    ...
    vm = tinypy_internal_value_vm(container);
    assert(tinypy_internal_vm_valid(vm));
    assert(tinypy_internal_value_belongs_to(vm, item));
    tinypy_internal_clear_error(out_error);
    if (container->type->sequence_slots != NULL && container->type->sequence_slots->contains != NULL) return container->type->sequence_slots->contains(container, item, out_error);
    kind = tinypy_internal_value_kind(container);
    if (kind == TINYPY_VALUE_DICT) return tinypy_dict_contains(container, item) != 0 ? 1 : 0;
    if (kind == TINYPY_VALUE_SET || kind == TINYPY_VALUE_FROZENSET) return tinypy_set_contains(container, item, out_error);
    if (kind == TINYPY_VALUE_STRING || kind == TINYPY_VALUE_UNICODE) return __tinypy_comparison_text_contains(container, item, out_error);
    if (kind == TINYPY_VALUE_BYTEARRAY) {
        int64_t integer;
        size_t size = tinypy_bytearray_size(container);
        size_t index;

        if (__tinypy_comparison_is_numeric(tinypy_internal_value_kind(item)) == 0 || (tinypy_internal_value_kind(item) == TINYPY_VALUE_FLOAT || tinypy_internal_value_kind(item) == TINYPY_VALUE_COMPLEX)) {
            tinypy_internal_make_vm_error(vm, TINYPY_ERROR_TYPE, "bytearray containment requires an integer", out_error);
            return -1;
        }
        if (tinypy_internal_value_kind(item) == TINYPY_VALUE_BOOL || tinypy_internal_value_kind(item) == TINYPY_VALUE_INTEGER) integer = TINYPY_INTEGER_VALUE(item);
        else {
            if (TINYPY_LONG_DIGIT_COUNT(item) > 1U || TINYPY_LONG_SIGN(item) < 0) return 0;
            integer = TINYPY_LONG_DIGIT_COUNT(item) == 0U ? 0 : (int64_t)TINYPY_LONG_OBJECT(item)->digits[0];
        }
        if (integer < 0 || integer > 255) return 0;
        for (index = 0U; index < size; index += 1U) if (TINYPY_BYTEARRAY_OBJECT(container)->bytes[index] == (unsigned char)integer) return 1;
        return 0;
    }
    if (kind == TINYPY_VALUE_TUPLE || kind == TINYPY_VALUE_LIST) {
        size_t size = kind == TINYPY_VALUE_TUPLE ? tinypy_tuple_size(container) : tinypy_list_size(container);
        size_t index;

        for (index = 0U; index < size; index += 1U) {
            tinypy_value_t *candidate = kind == TINYPY_VALUE_TUPLE ? tinypy_tuple_get(container, index) : tinypy_list_get(container, index);

            if (tinypy_internal_equal_value(candidate, item, 1) != 0) return 1;
        }
        return 0;
    }
...
static int32_t __tinypy_comparison_try_special(tinypy_value_t *left, tinypy_value_t *right, tinypy_compare_operation_e operation, int32_t *out_handled, int32_t *out_value, tinypy_error_t **out_error)
{
    ...
    *out_handled = INT32_C(0);
    if (operation > TINYPY_COMPARE_GREATER_EQUAL) return INT32_C(1);
    ...
    if ((right->type != left->type || (tinypy_internal_value_kind(left) == TINYPY_VALUE_OLD_INSTANCE && tinypy_old_instance_class(left) != tinypy_old_instance_class(right))) && tinypy_internal_object_has_special(right, right_names[index], name_sizes[index]) != 0) {
    ...
    if (tinypy_internal_object_has_special(left, "__cmp__", 7U) != 0) {
        ...
        if (tinypy_internal_value_kind(result) == TINYPY_VALUE_BOOL || tinypy_internal_value_kind(result) == TINYPY_VALUE_INTEGER) order = TINYPY_INTEGER_VALUE(result);
        else {
            tinypy_release(result);
        ...
        tinypy_release(result);
        if (operation == TINYPY_COMPARE_LESS) *out_value = order < 0;
        else if (operation == TINYPY_COMPARE_LESS_EQUAL) *out_value = order <= 0;
        else if (operation == TINYPY_COMPARE_EQUAL) *out_value = order == 0;
        else if (operation == TINYPY_COMPARE_NOT_EQUAL) *out_value = order != 0;
        else if (operation == TINYPY_COMPARE_GREATER) *out_value = order > 0;
        else *out_value = order >= 0;
        *out_handled = INT32_C(1);
    }
    return INT32_C(1);
}
...
int32_t tinypy_compare_bool(tinypy_value_t *left, tinypy_value_t *right, tinypy_compare_operation_e operation, tinypy_error_t **out_error)
{
    ...
    tinypy_internal_clear_error(out_error);
    if (operation == TINYPY_COMPARE_IS) return left == right;
    if (operation == TINYPY_COMPARE_IS_NOT) return left != right;
    ...
    if (operation <= TINYPY_COMPARE_GREATER_EQUAL) {
        ...
        if (__tinypy_comparison_try_special(left, right, operation, &handled, &special_value, out_error) == 0) return -1;
        if (handled != 0) return special_value;
    }
    if (operation == TINYPY_COMPARE_EQUAL) return tinypy_equal(left, right);
    if (operation == TINYPY_COMPARE_NOT_EQUAL) return tinypy_equal(left, right) == 0;
    if (__tinypy_comparison_order(left, right, &order, &unordered, out_error) == 0) return -1;
    if (unordered != 0) return 0;
    if (operation == TINYPY_COMPARE_LESS) return order < 0;
    if (operation == TINYPY_COMPARE_LESS_EQUAL) return order <= 0;
    if (operation == TINYPY_COMPARE_GREATER) return order > 0;
    return order >= 0;
}
