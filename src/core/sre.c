...
{
    if (character >= (uint32_t)'A' && character <= (uint32_t)'Z') return character + (uint32_t)('a' - 'A');
    return character;
}
...
static int32_t __tinypy_sre_category(uint32_t category, uint32_t character)
{
    ...
    case TINYPY_SRE_CATEGORY_UNI_DIGIT: return __tinypy_sre_is_digit(character);
    case TINYPY_SRE_CATEGORY_NOT_DIGIT:
    case TINYPY_SRE_CATEGORY_UNI_NOT_DIGIT: return __tinypy_sre_is_digit(character) == 0;
    case TINYPY_SRE_CATEGORY_SPACE:
    case TINYPY_SRE_CATEGORY_UNI_SPACE: return __tinypy_sre_is_space(character);
    case TINYPY_SRE_CATEGORY_NOT_SPACE:
    case TINYPY_SRE_CATEGORY_UNI_NOT_SPACE: return __tinypy_sre_is_space(character) == 0;
    case TINYPY_SRE_CATEGORY_WORD:
    case TINYPY_SRE_CATEGORY_LOC_WORD:
    case TINYPY_SRE_CATEGORY_UNI_WORD: return __tinypy_sre_is_word(character);
    case TINYPY_SRE_CATEGORY_NOT_WORD:
    case TINYPY_SRE_CATEGORY_LOC_NOT_WORD:
    case TINYPY_SRE_CATEGORY_UNI_NOT_WORD: return __tinypy_sre_is_word(character) == 0;
    case TINYPY_SRE_CATEGORY_LINEBREAK:
    case TINYPY_SRE_CATEGORY_UNI_LINEBREAK: return __tinypy_sre_is_linebreak(character);
    case TINYPY_SRE_CATEGORY_NOT_LINEBREAK:
    case TINYPY_SRE_CATEGORY_UNI_NOT_LINEBREAK: return __tinypy_sre_is_linebreak(character) == 0;
    default: return INT32_C(0);
    }
}
...
static int32_t __tinypy_sre_at(const tinypy_sre_state_t *state, size_t position, uint32_t at)
{
    ...
    case TINYPY_SRE_AT_BEGINNING_STRING: return position == state->beginning;
    case TINYPY_SRE_AT_BEGINNING_LINE: return position == state->beginning || __tinypy_sre_is_linebreak(state->bytes[position - 1U]) != 0;
    case TINYPY_SRE_AT_END: return position == state->end || (position + 1U == state->end && __tinypy_sre_is_linebreak(state->bytes[position]) != 0);
    case TINYPY_SRE_AT_END_LINE: return position == state->end || __tinypy_sre_is_linebreak(state->bytes[position]) != 0;
    case TINYPY_SRE_AT_END_STRING: return position == state->end;
    ...
    case TINYPY_SRE_AT_UNI_NON_BOUNDARY:
        ...
        if (at == TINYPY_SRE_AT_BOUNDARY || at == TINYPY_SRE_AT_LOC_BOUNDARY || at == TINYPY_SRE_AT_UNI_BOUNDARY) return previous_word != current_word;
...
static int32_t __tinypy_sre_charset(const tinypy_sre_pattern_object_t *pattern, size_t pc, uint32_t character)
{
    ...
    while (pc < pattern->code_size) {
        ...
        case TINYPY_SRE_OP_FAILURE: return accepted == 0;
        case TINYPY_SRE_OP_LITERAL:
            if (pc >= pattern->code_size) return INT32_C(0);
            if (character == pattern->code[pc]) return accepted;
            pc += 1U;
            break;
        case TINYPY_SRE_OP_CATEGORY:
            if (pc >= pattern->code_size) return INT32_C(0);
            if (__tinypy_sre_category(pattern->code[pc], character) != 0) return accepted;
            pc += 1U;
            break;
        case TINYPY_SRE_OP_CHARSET:
            if (pc + 8U > pattern->code_size) return INT32_C(0);
            if (character < UINT32_C(256) && (pattern->code[pc + (character >> 5U)] & (UINT32_C(1) << (character & UINT32_C(31)))) != 0U) return accepted;
            pc += 8U;
            break;
        case TINYPY_SRE_OP_RANGE:
            if (pc + 2U > pattern->code_size) return INT32_C(0);
            if (pattern->code[pc] <= character && character <= pattern->code[pc + 1U]) return accepted;
            pc += 2U;
            break;
        ...
        case TINYPY_SRE_OP_BIGCHARSET:
            {
                ...
                if (pc + 64U + block_count * 8U > pattern->code_size) return INT32_C(0);
                block_indices = (const unsigned char *)(pattern->code + pc);
                block = character <= UINT32_C(65535) ? block_indices[character >> 8U] : SIZE_MAX;
                pc += 64U;
                if (block != SIZE_MAX && block < block_count && (pattern->code[pc + block * 8U + ((character & UINT32_C(255)) >> 5U)] & (UINT32_C(1) << (character & UINT32_C(31)))) != 0U) return accepted;
                pc += block_count * 8U;
            }
            break;
        default: return INT32_C(0);
...
{
    if (count != 0U) (void)memcpy(target, source, count * sizeof(*target));
}
...
static int32_t __tinypy_sre_match_one(tinypy_sre_state_t *state, size_t pc, size_t position, size_t *out_position)
{
    ...
    if (position >= state->end || pc >= state->pattern->code_size) return INT32_C(0);
    opcode = state->pattern->code[pc];
    character = state->bytes[position];
    switch ((tinypy_sre_opcode_e)opcode) {
    case TINYPY_SRE_OP_LITERAL:
        if (pc + 1U >= state->pattern->code_size || character != state->pattern->code[pc + 1U]) return INT32_C(0);
        break;
    case TINYPY_SRE_OP_LITERAL_IGNORE:
        if (pc + 1U >= state->pattern->code_size || __tinypy_sre_lower(character) != __tinypy_sre_lower(state->pattern->code[pc + 1U])) return INT32_C(0);
        break;
    ...
    case TINYPY_SRE_OP_ANY:
        if (__tinypy_sre_is_linebreak(character) != 0) return INT32_C(0);
        break;
    case TINYPY_SRE_OP_ANY_ALL:
        break;
    case TINYPY_SRE_OP_IN:
        if (pc + 1U >= state->pattern->code_size || __tinypy_sre_charset(state->pattern, pc + 2U, character) == 0) return INT32_C(0);
        break;
    case TINYPY_SRE_OP_IN_IGNORE:
        if (pc + 1U >= state->pattern->code_size || __tinypy_sre_charset(state->pattern, pc + 2U, __tinypy_sre_lower(character)) == 0) return INT32_C(0);
        break;
    case TINYPY_SRE_OP_CATEGORY:
        if (pc + 1U >= state->pattern->code_size || __tinypy_sre_category(state->pattern->code[pc + 1U], character) == 0) return INT32_C(0);
        break;
    default: return INT32_C(0);
    }
...
static int32_t __tinypy_sre_match_repeat(tinypy_sre_state_t *state, size_t pc, size_t stop, size_t *position, size_t *marks, ptrdiff_t *lastindex)
{
    ...
    if (until >= pattern->code_size || capacity > SIZE_MAX / sizeof(*positions) || pattern->groups * 2U > TINYPY_SRE_MAX_MARKS) return INT32_C(0);
    greedy = pattern->code[until] == TINYPY_SRE_OP_MAX_UNTIL ? INT32_C(1) : INT32_C(0);
    if (greedy == 0 && pattern->code[until] != TINYPY_SRE_OP_MIN_UNTIL) return INT32_C(0);
    ...
    if (pattern->groups != 0U) __tinypy_sre_copy_marks(saved_marks, marks, pattern->groups * 2U);
    while (count < maximum) {
        size_t next_position = positions[count];
        size_t next_marks[TINYPY_SRE_MAX_MARKS];
        ptrdiff_t next_lastindex = saved_lastindex[count];

        if (pattern->groups != 0U) __tinypy_sre_copy_marks(next_marks, saved_marks + count * pattern->groups * 2U, pattern->groups * 2U);
        if (__tinypy_sre_match_code(state, pc + 4U, until, &next_position, next_marks, &next_lastindex) == 0 || next_position == positions[count]) break;
        count += 1U;
        positions[count] = next_position;
        saved_lastindex[count] = next_lastindex;
        if (pattern->groups != 0U) __tinypy_sre_copy_marks(saved_marks + count * pattern->groups * 2U, next_marks, pattern->groups * 2U);
    }
    if (count >= minimum) {
        ...
        for (index = 0U; index < attempts; index += 1U) {
            ...
            if (pattern->groups != 0U) __tinypy_sre_copy_marks(trial_marks, saved_marks + selected * pattern->groups * 2U, pattern->groups * 2U);
            if (__tinypy_sre_match_code(state, until + 1U, stop, &trial_position, trial_marks, &trial_lastindex) != 0) {
                ...
                *lastindex = trial_lastindex;
                if (pattern->groups != 0U) __tinypy_sre_copy_marks(marks, trial_marks, pattern->groups * 2U);
                if (saved_marks != NULL) tinypy_internal_vm_deallocate(state->vm, saved_marks, capacity * pattern->groups * 2U * sizeof(*saved_marks), (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
...
static int32_t __tinypy_sre_match_code(tinypy_sre_state_t *state, size_t pc, size_t stop, size_t *position, size_t *marks, ptrdiff_t *lastindex)
{
    ...
    while (pc < pattern->code_size) {
        ...
        case TINYPY_SRE_OP_JUMP:
            if (pc + 1U >= pattern->code_size || pattern->code[pc + 1U] > SIZE_MAX - pc - 1U) goto invalid;
        ...
        case TINYPY_SRE_OP_IN_IGNORE:
            {
                ...
                if (skip > SIZE_MAX - pc - 1U) goto invalid;
                *position = next_position;
                pc += 1U + skip;
            }
            break;
        case TINYPY_SRE_OP_MARK:
            {
                ...
                if (pc + 1U >= pattern->code_size) goto invalid;
                mark = pattern->code[pc + 1U];
                if (mark >= pattern->groups * 2U) goto invalid;
                marks[mark] = *position;
                if ((mark & 1U) != 0U) *lastindex = (ptrdiff_t)(mark / 2U + 1U);
                pc += 2U;
            }
            break;
        ...
        case TINYPY_SRE_OP_BRANCH:
            {
                ...
                while (branch < pattern->code_size && pattern->code[branch] != 0U) {
                    ...
                    if (skip > SIZE_MAX - branch) goto invalid;
                    if (pattern->groups != 0U) __tinypy_sre_copy_marks(trial_marks, marks, pattern->groups * 2U);
        ...
        case TINYPY_SRE_OP_MIN_REPEAT_ONE:
            {
                ...
                if (pc + 3U >= pattern->code_size) goto invalid;
                skip = pattern->code[pc + 1U];
                ...
                while (count < maximum) {
                    ...
                    if (__tinypy_sre_match_one(state, pc + 4U, cursor, &next_position) == 0) break;
        ...
        case TINYPY_SRE_OP_GROUPREF_EXISTS:
            {
                ...
                if (pc + 2U >= pattern->code_size) goto invalid;
                group = pattern->code[pc + 1U];
                mark = group * 2U;
                if (mark + 1U >= pattern->groups * 2U || marks[mark] == SIZE_MAX || marks[mark + 1U] == SIZE_MAX) pc += 1U + pattern->code[pc + 2U];
                else pc += 3U;
            }
            break;
        ...
        case TINYPY_SRE_OP_ASSERT_NOT:
            {
                ...
                if (back > *position - state->beginning || skip > SIZE_MAX - pc - 1U) matched = INT32_C(0);
                else {
...
static tinypy_value_t *__tinypy_sre_text_slice(tinypy_vm_t *vm, tinypy_value_t *text, size_t start, size_t end)
{
    ...
    if (tinypy_internal_value_kind(text) == TINYPY_VALUE_UNICODE) return tinypy_unicode_from_utf8(vm, (const char *)bytes + start, end - start);
...
void tinypy_internal_sre_pattern_destroy(tinypy_vm_t *vm, tinypy_value_t *value)
{
    ...
    if (pattern->code != NULL) tinypy_internal_vm_deallocate(vm, pattern->code, pattern->code_size * sizeof(*pattern->code), (uint32_t)TINYPY_ALLOC_TAG_SRE_DATA);
}
...
void tinypy_internal_sre_match_destroy(tinypy_vm_t *vm, tinypy_value_t *value)
{
    ...
    if (match->marks != NULL) tinypy_internal_vm_deallocate(vm, match->marks, match->mark_count * sizeof(*match->marks), (uint32_t)TINYPY_ALLOC_TAG_SRE_DATA);
}
...
static tinypy_value_t *__tinypy_sre_execute(tinypy_sre_pattern_object_t *pattern, tinypy_value_t *string, size_t pos, size_t endpos, int32_t search, tinypy_error_t **out_error)
{
    ...
    if (pos > string_size) pos = string_size;
    if (endpos > string_size) endpos = string_size;
    ...
    for (candidate = pos; candidate <= endpos; candidate += 1U) {
        ...
        for (index = 0U; index < pattern->groups * 2U; index += 1U) marks[index] = SIZE_MAX;
        state.invalid_code = INT32_C(0);
        if (__tinypy_sre_match_code(&state, 0U, SIZE_MAX, &matched_end, marks, &lastindex) != 0) return __tinypy_sre_match_new(&state, string, pos, endpos, candidate, matched_end, marks, lastindex);
        if (state.invalid_code != 0) {
            tinypy_internal_make_vm_error(vm, TINYPY_ERROR_RUNTIME, "invalid SRE bytecode", out_error);
            return NULL;
        }
        if (search == 0 || candidate == endpos) break;
    }
...
static int32_t __tinypy_sre_bounds(tinypy_sre_pattern_object_t *pattern, tinypy_value_t *args, size_t string_index, size_t *out_pos, size_t *out_endpos, tinypy_error_t **out_error)
{
    ...
    *out_endpos = size;
    if (tinypy_tuple_size(args) > string_index + 1U) {
        if (__tinypy_sre_integer(tinypy_tuple_get(args, string_index + 1U), &value) == 0) {
    ...
    if (tinypy_tuple_size(args) > string_index + 2U) {
        if (__tinypy_sre_integer(tinypy_tuple_get(args, string_index + 2U), &value) == 0) {
            tinypy_internal_make_vm_error(vm, TINYPY_ERROR_TYPE, "slice indices must be integers", out_error);
            return INT32_C(0);
        }
...
static tinypy_value_t *__tinypy_sre_pattern_match_or_search(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 2U, 4U, out_error) == 0 || tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_SRE_PATTERN) return NULL;
    pattern = TINYPY_SRE_PATTERN_OBJECT(tinypy_tuple_get(args, 0U));
    if (__tinypy_sre_bounds(pattern, args, 1U, &pos, &endpos, out_error) == 0) return NULL;
    return __tinypy_sre_execute(pattern, tinypy_tuple_get(args, 1U), pos, endpos, user_data != NULL ? INT32_C(1) : INT32_C(0), out_error);
}
...
static int32_t __tinypy_sre_match_group_index(tinypy_sre_match_object_t *match, tinypy_value_t *index_value, size_t *out_index, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_sre_integer(index_value, &index) == 0) {
        if (tinypy_dict_contains(pattern->groupindex, index_value) == 0 || __tinypy_sre_integer(tinypy_dict_get(pattern->groupindex, index_value), &index) == 0) {
...
static tinypy_value_t *__tinypy_sre_match_group_value(tinypy_sre_match_object_t *match, size_t index, tinypy_value_t *default_value)
{
    ...
    if (index == 0U) return __tinypy_sre_text_slice(vm, match->string, match->start, match->end);
...
static tinypy_value_t *__tinypy_sre_match_group(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 1U, SIZE_MAX, out_error) == 0 || tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_SRE_MATCH) return NULL;
    ...
    if (count <= 1U) {
        ...
        if (count == 1U && __tinypy_sre_match_group_index(match, tinypy_tuple_get(args, 1U), &index, out_error) == 0) {
            tinypy_release(none);
            return NULL;
        }
    ...
    }
    {
        ...
        for (item_index = 0U; item_index < count; item_index += 1U) {
            ...
            if (__tinypy_sre_match_group_index(match, tinypy_tuple_get(args, item_index + 1U), &group, out_error) == 0) {
                while (item_index != 0U) tinypy_release(items[--item_index]);
...
static tinypy_value_t *__tinypy_sre_match_groups(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 1U, 2U, out_error) == 0 || tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_SRE_MATCH) return NULL;
    match = TINYPY_SRE_MATCH_OBJECT(tinypy_tuple_get(args, 0U));
    ...
    if (pattern->groups == 0U) return tinypy_tuple_from_items(vm, NULL, 0U);
    items = (tinypy_value_t **)tinypy_internal_vm_allocate(vm, pattern->groups * sizeof(*items), (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    for (index = 0U; index < pattern->groups; index += 1U) items[index] = __tinypy_sre_match_group_value(match, index + 1U, default_value);
    result = tinypy_tuple_from_items(vm, items, pattern->groups);
    for (index = 0U; index < pattern->groups; index += 1U) tinypy_release(items[index]);
...
static int32_t __tinypy_sre_match_span_value(tinypy_sre_match_object_t *match, tinypy_value_t *args, size_t *out_start, size_t *out_end, tinypy_error_t **out_error)
{
    ...
    if (tinypy_tuple_size(args) == 2U && __tinypy_sre_match_group_index(match, tinypy_tuple_get(args, 1U), &group, out_error) == 0) return INT32_C(0);
    if (group == 0U) {
...
static tinypy_value_t *__tinypy_sre_match_span_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 1U, 2U, out_error) == 0 || tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_SRE_MATCH) return NULL;
    match = TINYPY_SRE_MATCH_OBJECT(tinypy_tuple_get(args, 0U));
    if (__tinypy_sre_match_span_value(match, args, &start, &end, out_error) == 0) return NULL;
    if (operation == 0) return tinypy_integer_from_i64(vm, start == SIZE_MAX ? INT64_C(-1) : (int64_t)start);
    if (operation == 1) return tinypy_integer_from_i64(vm, end == SIZE_MAX ? INT64_C(-1) : (int64_t)end);
    {
...
static tinypy_value_t *__tinypy_sre_pattern_findall(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    while (pos <= endpos) {
        ...
        if (pattern->groups == 0U) item = __tinypy_sre_text_slice(vm, string, match->start, match->end);
            ...
            for (index = 0U; index < pattern->groups; index += 1U) items[index] = __tinypy_sre_match_group_value(match, index + 1U, empty);
...
static tinypy_value_t *__tinypy_sre_join(tinypy_vm_t *vm, tinypy_value_t *pieces, tinypy_value_t *source, tinypy_error_t **out_error)
{
    ...
    for (index = 0U; index < count; index += 1U) {
        ...
        if (kind == TINYPY_VALUE_UNICODE) unicode = INT32_C(1);
        size = tinypy_internal_text_byte_size(piece);
        assert(size <= SIZE_MAX - total);
        total += size;
    }
    if (total == 0U) return unicode != 0 ? tinypy_unicode_from_utf8(vm, NULL, 0U) : tinypy_string_from_bytes(vm, NULL, 0U);
    ...
    for (index = 0U; index < count; index += 1U) {
        ...
        if (size != 0U) (void)memcpy(bytes + offset, tinypy_internal_text_bytes(piece), size);
        offset += size;
    }
...
static tinypy_value_t *__tinypy_sre_pattern_sub(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 3U, 4U, out_error) == 0 || tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_SRE_PATTERN) return NULL;
    pattern = TINYPY_SRE_PATTERN_OBJECT(tinypy_tuple_get(args, 0U));
    ...
    if (tinypy_tuple_size(args) == 4U) {
        ...
        if (__tinypy_sre_integer(tinypy_tuple_get(args, 3U), &count) == 0) {
    ...
    while (pos <= string_size && (limit == 0U || substitutions < limit)) {
        ...
        if (tinypy_internal_value_kind(piece) != TINYPY_VALUE_NONE) tinypy_list_append(pieces, piece);
        tinypy_release(piece);
        copied = match->end;
        substitutions += 1U;
        pos = match->end == match->start ? match->end + 1U : match->end;
    ...
    if (user_data == NULL) return joined;
    {
...
static tinypy_value_t *__tinypy_sre_compile(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 6U, 6U, out_error) == 0) return NULL;
    ...
    if ((tinypy_internal_value_kind(source) != TINYPY_VALUE_NONE && tinypy_internal_value_kind(source) != TINYPY_VALUE_STRING && tinypy_internal_value_kind(source) != TINYPY_VALUE_UNICODE) || __tinypy_sre_integer(tinypy_tuple_get(args, 1U), &flags) == 0 || __tinypy_sre_integer(tinypy_tuple_get(args, 3U), &groups) == 0 || groups < 0 || groups > 100 || tinypy_internal_value_kind(groupindex) != TINYPY_VALUE_DICT || tinypy_internal_value_kind(indexgroup) != TINYPY_VALUE_LIST || (tinypy_internal_value_kind(code_value) != TINYPY_VALUE_LIST && tinypy_internal_value_kind(code_value) != TINYPY_VALUE_TUPLE)) {
...
static tinypy_value_t *__tinypy_sre_getlower(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_sre_method_arguments(vm, args, kwargs, 2U, 2U, out_error) == 0 || __tinypy_sre_integer(tinypy_tuple_get(args, 0U), &character) == 0 || character < 0 || (uint64_t)character > UINT32_MAX) {
        tinypy_internal_make_vm_error(vm, TINYPY_ERROR_TYPE, "getlower arguments are invalid", out_error);
        return NULL;
    }
    return tinypy_integer_from_i64(vm, (int64_t)__tinypy_sre_lower((uint32_t)character));
}
