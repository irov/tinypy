...
{
    if (value == NULL) return tinypy_none_get(vm);
    tinypy_retain(value);
    return value;
}
...
static void __tinypy_internal_c_descriptor_replace(tinypy_value_t **target, tinypy_value_t *value)
{
    ...
    if (value != NULL) tinypy_retain(value);
    *target = value;
    if (previous != NULL) tinypy_release(previous);
}
...
tinypy_value_t *tinypy_property_new(tinypy_vm_t *vm, tinypy_value_t *getter, tinypy_value_t *setter, tinypy_value_t *deleter, tinypy_value_t *doc)
{
    ...
    property->doc = doc;
    if (getter != NULL) tinypy_retain(getter);
    if (setter != NULL) tinypy_retain(setter);
    if (deleter != NULL) tinypy_retain(deleter);
    if (doc != NULL) tinypy_retain(doc);
...
void tinypy_internal_property_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    if (property->getter != NULL) visit(property->getter, user_data);
    if (property->setter != NULL) visit(property->setter, user_data);
    if (property->deleter != NULL) visit(property->deleter, user_data);
    if (property->doc != NULL) visit(property->doc, user_data);
}
...
static tinypy_value_t *__tinypy_internal_descriptor_constructor(tinypy_type_t *type, tinypy_value_t *args, tinypy_value_t *kwargs, tinypy_error_t **out_error, int class_method)
{
    ...
    return class_method != 0 ? tinypy_class_method_new(tinypy_tuple_get(args, 0U)) : tinypy_static_method_new(tinypy_tuple_get(args, 0U));
}
...
tinypy_value_t *tinypy_internal_property_create(tinypy_type_t *type, tinypy_value_t *args, tinypy_value_t *kwargs, tinypy_error_t **out_error)
{
    ...
    if (values[3] == NULL && values[0] != NULL && tinypy_internal_value_kind(values[0]) == TINYPY_VALUE_FUNCTION) values[3] = tinypy_function_doc(values[0]);
...
static int __tinypy_internal_property_method_arguments(tinypy_vm_t *vm, tinypy_value_t *args, tinypy_value_t *kwargs, size_t count, tinypy_error_t **out_error)
{
    ...
    if (tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_PROPERTY) {
...
static tinypy_value_t *__tinypy_internal_property_copy(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, int field, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_internal_property_method_arguments(vm, args, kwargs, 2U, out_error) == 0) return NULL;
    property = TINYPY_PROPERTY_OBJECT(tinypy_tuple_get(args, 0U));
    replacement = tinypy_tuple_get(args, 1U);
    if (tinypy_internal_value_kind(replacement) == TINYPY_VALUE_NONE) replacement = NULL;
...
static tinypy_value_t *__tinypy_internal_property_field(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, int field, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_internal_property_method_arguments(vm, args, kwargs, 1U, out_error) == 0) return NULL;
    property = TINYPY_PROPERTY_OBJECT(tinypy_tuple_get(args, 0U));
    switch (field) {
    case 0: result = property->getter; break;
    case 1: result = property->setter; break;
    case 2: result = property->deleter; break;
    default: result = property->doc; break;
    }
    if (result == NULL) return tinypy_none_get(vm);
...
void tinypy_internal_c_descriptor_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    if (descriptor->owner_retained != 0) visit(&descriptor->owner->base.base, user_data);
...
int32_t tinypy_internal_c_descriptor_set(tinypy_value_t *descriptor_value, tinypy_value_t *instance, tinypy_value_t *value, tinypy_error_t **out_error)
{
    ...
    case TINYPY_INTERNAL_C_DESCRIPTOR_FUNCTION_CODE:
        ...
        if (function->closure != NULL && tinypy_tuple_size(function->closure) != tinypy_tuple_size(tinypy_code_freevars(value))) {
        ...
        return INT32_C(1);
    case TINYPY_INTERNAL_C_DESCRIPTOR_FUNCTION_DEFAULTS:
        if (value != NULL && tinypy_internal_value_kind(value) == TINYPY_VALUE_NONE) value = NULL;
    ...
    case TINYPY_INTERNAL_C_DESCRIPTOR_FUNCTION_NAME:
        ...
        return INT32_C(1);
    case TINYPY_INTERNAL_C_DESCRIPTOR_FUNCTION_DOC:
        if (value == NULL) value = tinypy_none_get(vm);
        else tinypy_retain(value);
...
tinypy_value_t *tinypy_property_getter(const tinypy_value_t *property) { return __tinypy_internal_property_validate(property)->getter; }
tinypy_value_t *tinypy_property_setter(const tinypy_value_t *property) { return __tinypy_internal_property_validate(property)->setter; }
tinypy_value_t *tinypy_property_deleter(const tinypy_value_t *property) { return __tinypy_internal_property_validate(property)->deleter; }
tinypy_value_t *tinypy_property_doc(const tinypy_value_t *property) { return __tinypy_internal_property_validate(property)->doc; }
