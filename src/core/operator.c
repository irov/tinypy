...
//////////////////////////////////////////////////////////////////////////
static void __tinypy_operator_long_multiply_digits(tinypy_vm_t *vm, uint16_t *output, size_t output_capacity, const uint16_t *left, size_t left_count, const uint16_t *right, size_t right_count) {
    ...
    size_t zero_low_capacity = left_low_count + right_low_count;
    size_t zero_high_capacity = left_high_count + right_high_count;
    size_t left_sum_capacity = (left_low_count > left_high_count ? left_low_count : left_high_count) + 1U;
    size_t right_sum_capacity = (right_low_count > right_high_count ? right_low_count : right_high_count) + 1U;
    uint16_t *zero_low = (uint16_t *)tinypy_internal_vm_allocate(vm, zero_low_capacity * sizeof(*zero_low));
    uint16_t *zero_high = (uint16_t *)tinypy_internal_vm_allocate(vm, zero_high_capacity * sizeof(*zero_high));
    uint16_t *left_sum = (uint16_t *)tinypy_internal_vm_allocate(vm, left_sum_capacity * sizeof(*left_sum));
    uint16_t *right_sum = (uint16_t *)tinypy_internal_vm_allocate(vm, right_sum_capacity * sizeof(*right_sum));
    ...
    (void)memset(zero_low, 0, zero_low_capacity * sizeof(*zero_low));
    (void)memset(zero_high, 0, zero_high_capacity * sizeof(*zero_high));
    ...
    cross_capacity = left_sum_count + right_sum_count;
    cross = (uint16_t *)tinypy_internal_vm_allocate(vm, cross_capacity * sizeof(*cross));
    (void)memset(cross, 0, cross_capacity * sizeof(*cross));

    __tinypy_operator_long_multiply_digits(vm, zero_low, zero_low_capacity, left, left_low_count, right, right_low_count);
    __tinypy_operator_long_multiply_digits(vm, zero_high, zero_high_capacity, left + split, left_high_count, right + split, right_high_count);
    __tinypy_operator_long_multiply_digits(vm, cross, cross_capacity, left_sum, left_sum_count, right_sum, right_sum_count);
    ...
    tinypy_internal_vm_deallocate(vm, cross, cross_capacity * sizeof(*cross));
    tinypy_internal_vm_deallocate(vm, right_sum, right_sum_capacity * sizeof(*right_sum));
    tinypy_internal_vm_deallocate(vm, left_sum, left_sum_capacity * sizeof(*left_sum));
    tinypy_internal_vm_deallocate(vm, zero_high, zero_high_capacity * sizeof(*zero_high));
    tinypy_internal_vm_deallocate(vm, zero_low, zero_low_capacity * sizeof(*zero_low));
}
//////////////////////////////////////////////////////////////////////////
static tinypy_value_t *__tinypy_operator_long_multiply_views(tinypy_vm_t *vm, const tinypy_integer_view_t *left, const tinypy_integer_view_t *right) {
    ...
    __tinypy_operator_long_multiply_digits(vm, digits, capacity, left->digits, left->count, right->digits, right->count);
...
static tinypy_value_t *__tinypy_operator_long_divide_views(tinypy_vm_t *vm, const tinypy_integer_view_t *left, const tinypy_integer_view_t *right, tinypy_operator_long_division_result_e result_kind, tinypy_error_t **out_error) {
    ...
    if (left->sign == 0) {
        ...
        uint16_t *normalized_divisor = (uint16_t *)tinypy_internal_vm_allocate(vm, divisor_count * sizeof(*normalized_divisor));
        uint16_t *normalized_dividend = (uint16_t *)tinypy_internal_vm_allocate(vm, (dividend_count + 1U) * sizeof(*normalized_dividend));
        uint32_t carry = 0U;
        size_t index;
        ...
        tinypy_internal_vm_deallocate(vm, normalized_dividend, (dividend_count + 1U) * sizeof(*normalized_dividend));
        tinypy_internal_vm_deallocate(vm, normalized_divisor, divisor_count * sizeof(*normalized_divisor));
    }
    ...
    if (result_kind == TINYPY_OPERATOR_LONG_DIVISION_PAIR) {
        ...
        items[0] = tinypy_long_from_base15_digits(vm, quotient_sign, quotient, quotient_count);
        items[1] = tinypy_long_from_base15_digits(vm, remainder_sign, remainder_digits, remainder_count);
        ...
        result = tinypy_long_from_base15_digits(vm, remainder_sign, remainder_digits, remainder_count);
    }
    else {
        result = tinypy_long_from_base15_digits(vm, quotient_sign, quotient, quotient_count);
    }
...
static tinypy_value_t *__tinypy_operator_concat_sequence(tinypy_vm_t *vm, tinypy_value_t *left, tinypy_value_t *right, tinypy_error_t **out_error) {
    ...
    size_t total_size;
    size_t index;
    tinypy_value_t *empty_item = NULL;
    tinypy_value_t **items = &empty_item;
    ...
    if (total_size != 0U) {
        items = (tinypy_value_t **)tinypy_internal_vm_allocate(vm, total_size * sizeof(*items));
    }
    ...
    tinypy_value_t *result = kind == TINYPY_VALUE_TUPLE ? tinypy_tuple_from_items(vm, items, total_size) : tinypy_list_from_items(vm, items, total_size);
    if (total_size != 0U) {
...
static tinypy_value_t *__tinypy_operator_repeat(tinypy_vm_t *vm, tinypy_value_t *sequence, size_t count, tinypy_error_t **out_error) {
    ...
    if (kind == TINYPY_VALUE_STRING || kind == TINYPY_VALUE_UNICODE) {
        ...
        uint8_t *buffer;
        tinypy_value_t *result = tinypy_internal_text_allocate_uninitialized(vm, kind, total_size, code_point_count, &buffer);
    ...
    total_size = unit_size * count; {
        ...
        tinypy_value_t *result = kind == TINYPY_VALUE_TUPLE ? tinypy_tuple_new(vm, total_size) : tinypy_list_from_items(vm, NULL, 0U);

        if (kind == TINYPY_VALUE_LIST) {
            tinypy_internal_list_reserve(vm, result, total_size);
        }
...
static tinypy_value_t *__tinypy_operator_divide(tinypy_value_t *left, tinypy_value_t *right, tinypy_operator_division_e division, tinypy_error_t **out_error) {
    ...
    if (__tinypy_operator_is_integer(left_kind) != 0 && __tinypy_operator_is_integer(right_kind) != 0) {
        ...
        if (dividend == INT64_MIN && divisor == -1) {
            if (division == TINYPY_OPERATOR_DIVISION_REMAINDER) {
                tinypy_value_t *return_value_7 = tinypy_integer_from_i64(vm, 0);
                return return_value_7;
            }
            tinypy_integer_view_t left_view;
            ...
            tinypy_value_t *return_value_8 = __tinypy_operator_long_multiply_views(vm, &left_view, &minus_one);
