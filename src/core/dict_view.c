...
int32_t tinypy_internal_dict_view_contains(tinypy_value_t *value, tinypy_value_t *item, tinypy_error_t **out_error) {
    ...
    if (view->kind == TINYPY_DICT_VIEW_ITEMS) {
        ...
        if (tinypy_internal_value_kind(item) != TINYPY_VALUE_TUPLE || tinypy_tuple_size(item) != 2U) {
            return INT32_C(0);
        }
    ...
    for (index = 0U; index <= dict->mask; index += 1U) {
        ...
        if (entry->state == TINYPY_DICT_ENTRY_ACTIVE && tinypy_equal(entry->value, item) != 0) {
            return INT32_C(1);
        }
    }
    return INT32_C(0);
