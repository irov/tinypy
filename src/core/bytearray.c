...
static int32_t __tinypy_bytearray_integer(tinypy_value_t *value, int64_t *out_value)
{
    ...
    if (kind == TINYPY_VALUE_LONG && TINYPY_LONG_DIGIT_COUNT(value) <= 5U) {
        ...
        while (index != 0U) {
            index -= 1U;
            if (magnitude > (UINT64_MAX >> 15U)) return INT32_C(0);
...
static void __tinypy_bytearray_reserve(tinypy_value_t *value, size_t minimum)
{
    ...
    if (minimum <= bytearray->capacity) return;
    ...
    if (bytearray->bytes == NULL) bytearray->bytes = (unsigned char *)tinypy_internal_vm_allocate(vm, capacity, (uint32_t)TINYPY_ALLOC_TAG_BYTEARRAY_DATA);
    else bytearray->bytes = (unsigned char *)tinypy_internal_vm_reallocate(vm, bytearray->bytes, bytearray->capacity, capacity, (uint32_t)TINYPY_ALLOC_TAG_BYTEARRAY_DATA);
    ...
    *out_size = 0U;
    ...
    for (;;) {
        ...
        if (__tinypy_bytearray_item(vm, item, &byte, out_error) == 0) {
            ...
            if (bytes != NULL) tinypy_internal_vm_deallocate(vm, bytes, capacity, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
            return INT32_C(0);
        }
        tinypy_release(item);
        if (size == capacity) {
            ...
            if (bytes == NULL) bytes = (unsigned char *)tinypy_internal_vm_allocate(vm, new_capacity, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
            else bytes = (unsigned char *)tinypy_internal_vm_reallocate(vm, bytes, capacity, new_capacity, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    ...
    if (size != 0U && capacity != size) bytes = (unsigned char *)tinypy_internal_vm_reallocate(vm, bytes, capacity, size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    *out_bytes = bytes;
    *out_size = size;
    return INT32_C(1);
}
...
{
    if (__tinypy_bytearray_integer(value, out_value) != 0) return INT32_C(1);
...
static int32_t __tinypy_bytearray_slice(tinypy_vm_t *vm, tinypy_value_t *slice_value, size_t size, tinypy_bytearray_slice_t *out_slice, tinypy_error_t **out_error)
{
    ...
    if (tinypy_internal_value_kind(step_value) == TINYPY_VALUE_NONE) out_slice->step = INT64_C(1);
    else if (__tinypy_bytearray_slice_integer(vm, step_value, &out_slice->step, out_error) == 0) return INT32_C(0);
    if (out_slice->step == 0) {
        tinypy_internal_make_vm_error(vm, TINYPY_ERROR_VALUE, "slice step cannot be zero", out_error);
        return INT32_C(0);
    }
    if (out_slice->step == INT64_MIN) out_slice->step = -INT64_MAX;
    lower = out_slice->step < 0 ? -1 : 0;
    upper = out_slice->step < 0 ? sequence_size - 1 : sequence_size;
    if (tinypy_internal_value_kind(start_value) == TINYPY_VALUE_NONE) out_slice->start = out_slice->step < 0 ? upper : lower;
    else {
        if (__tinypy_bytearray_slice_integer(vm, start_value, &out_slice->start, out_error) == 0) return INT32_C(0);
        if (out_slice->start < 0) out_slice->start += sequence_size;
        if (out_slice->start < lower) out_slice->start = lower;
        if (out_slice->start > upper) out_slice->start = upper;
    }
    if (tinypy_internal_value_kind(stop_value) == TINYPY_VALUE_NONE) out_slice->stop = out_slice->step < 0 ? lower : upper;
    else {
        if (__tinypy_bytearray_slice_integer(vm, stop_value, &out_slice->stop, out_error) == 0) return INT32_C(0);
        if (out_slice->stop < 0) out_slice->stop += sequence_size;
        if (out_slice->stop < lower) out_slice->stop = lower;
        if (out_slice->stop > upper) out_slice->stop = upper;
    }
    out_slice->length = 0U;
    if (out_slice->step < 0 && out_slice->stop < out_slice->start) out_slice->length = (size_t)(1 + (out_slice->start - out_slice->stop - 1) / -out_slice->step);
    else if (out_slice->step > 0 && out_slice->start < out_slice->stop) out_slice->length = (size_t)(1 + (out_slice->stop - out_slice->start - 1) / out_slice->step);
    return INT32_C(1);
}
...
{
    tinypy_bytearray_object_t *bytearray = TINYPY_BYTEARRAY_OBJECT(value);
    size_t old_size = (size_t)TINYPY_SIZE(value);
    size_t index;
    ...
    if (slice->step == 1) {
        ...
        if (tail_size != 0U && replacement_size != slice->length) (void)memmove(bytearray->bytes + (size_t)slice->start + replacement_size, bytearray->bytes + tail_start, tail_size);
        if (replacement_size != 0U) (void)memcpy(bytearray->bytes + (size_t)slice->start, replacement, replacement_size);
        TINYPY_SIZE(value) = (ptrdiff_t)new_size;
        return;
    }
    assert(replacement_size == slice->length);
    for (index = 0U; index < replacement_size; index += 1U) bytearray->bytes[(size_t)(slice->start + (int64_t)index * slice->step)] = replacement[index];
}
...
static void __tinypy_bytearray_delete_index(tinypy_value_t *value, size_t index)
{
    ...
    assert(index < size);
    if (index + 1U < size) (void)memmove(bytearray->bytes + index, bytearray->bytes + index + 1U, size - index - 1U);
    TINYPY_SIZE(value) -= 1;
}
...
tinypy_value_t *tinypy_bytearray_from_bytes(tinypy_vm_t *vm, const void *bytes, size_t size)
{
    ...
    bytearray->base.size = (ptrdiff_t)size;
...
size_t tinypy_bytearray_size(const tinypy_value_t *value)
{
    ...
    assert(tinypy_internal_value_kind(value) == TINYPY_VALUE_BYTEARRAY);
...
void tinypy_internal_bytearray_destroy(tinypy_vm_t *vm, tinypy_value_t *value)
{
    ...
    if (bytearray->bytes == NULL) return;
    ...
    bytearray->capacity = 0U;
    bytearray->base.size = 0;
}
...
tinypy_value_t *tinypy_internal_bytearray_create(tinypy_type_t *type, tinypy_value_t *args, tinypy_value_t *kwargs, tinypy_error_t **out_error)
{
    ...
    if (tinypy_tuple_size(args) == 0U) return tinypy_bytearray_from_bytes(vm, NULL, 0U);
    ...
    if (__tinypy_bytearray_integer(source, &requested_size) != 0) {
        ...
        if (requested_size != 0) {
            ...
            TINYPY_SIZE(result) = (ptrdiff_t)requested_size;
        }
        return result;
    }
    if (__tinypy_bytearray_collect(vm, source, &bytes, &size, out_error) == 0) return NULL;
    result = tinypy_bytearray_from_bytes(vm, bytes, size);
    if (bytes != NULL) tinypy_internal_vm_deallocate(vm, bytes, size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    return result;
}
...
tinypy_value_t *tinypy_internal_bytearray_get_item(tinypy_value_t *value, tinypy_value_t *key, tinypy_error_t **out_error)
{
    ...
    if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) {
        ...
        if (__tinypy_bytearray_slice(vm, key, size, &slice, out_error) == 0) return NULL;
        if (slice.length == 0U) return tinypy_bytearray_from_bytes(vm, NULL, 0U);
        if (slice.step == 1) return tinypy_bytearray_from_bytes(vm, TINYPY_BYTEARRAY_OBJECT(value)->bytes + (size_t)slice.start, slice.length);
        selected = (unsigned char *)tinypy_internal_vm_allocate(vm, slice.length, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
        for (selected_index = 0U; selected_index < slice.length; selected_index += 1U) selected[selected_index] = TINYPY_BYTEARRAY_OBJECT(value)->bytes[(size_t)(slice.start + (int64_t)selected_index * slice.step)];
    ...
    if (__tinypy_bytearray_index(vm, key, size, &index, out_error) == 0) return NULL;
...
int32_t tinypy_internal_bytearray_set_item(tinypy_value_t *value, tinypy_value_t *key, tinypy_value_t *item, tinypy_error_t **out_error)
{
    ...
    if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) {
        ...
        if (__tinypy_bytearray_slice(vm, key, size, &slice, out_error) == 0) return INT32_C(0);
        if (item != NULL) {
            if (__tinypy_bytearray_collect(vm, item, &replacement, &replacement_size, out_error) == 0) return INT32_C(0);
            if (slice.step != 1 && replacement_size != slice.length) {
                if (replacement != NULL) tinypy_internal_vm_deallocate(vm, replacement, replacement_size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
        ...
        if (slice.step > 0) {
            for (deletion_index = slice.length; deletion_index != 0U; deletion_index -= 1U) __tinypy_bytearray_delete_index(value, (size_t)(slice.start + (int64_t)(deletion_index - 1U) * slice.step));
        } else {
            for (deletion_index = 0U; deletion_index < slice.length; deletion_index += 1U) __tinypy_bytearray_delete_index(value, (size_t)(slice.start + (int64_t)deletion_index * slice.step));
        }
        return INT32_C(1);
    }
    if (__tinypy_bytearray_index(vm, key, size, &index, out_error) == 0) return INT32_C(0);
    if (item == NULL) {
...
{
    tinypy_internal_clear_error(out_error);
    return tinypy_string_from_bytes(tinypy_internal_value_vm(value), TINYPY_BYTEARRAY_OBJECT(value)->bytes, (size_t)TINYPY_SIZE(value));
}
...
static int32_t __tinypy_bytearray_method_arguments(tinypy_vm_t *vm, tinypy_value_t *args, tinypy_value_t *kwargs, size_t minimum, size_t maximum, tinypy_error_t **out_error)
{
    ...
    if (tinypy_internal_value_kind(tinypy_tuple_get(args, 0U)) != TINYPY_VALUE_BYTEARRAY) {
...
static tinypy_value_t *__tinypy_bytearray_add_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_bytearray_method_arguments(vm, args, kwargs, 2U, 2U, out_error) == 0) return NULL;
    left = tinypy_tuple_get(args, 0U);
    if (tinypy_internal_bytes_view(tinypy_tuple_get(args, 1U), &right_bytes, &right_size) == 0) {
    ...
    if (left_size + right_size == 0U) return tinypy_bytearray_from_bytes(vm, NULL, 0U);
    bytes = (unsigned char *)tinypy_internal_vm_allocate(vm, left_size + right_size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    if (left_size != 0U) (void)memcpy(bytes, TINYPY_BYTEARRAY_OBJECT(left)->bytes, left_size);
    if (right_size != 0U) (void)memcpy(bytes + left_size, right_bytes, right_size);
...
static tinypy_value_t *__tinypy_bytearray_append_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_bytearray_method_arguments(vm, args, kwargs, 2U, 2U, out_error) == 0) return NULL;
    if (__tinypy_bytearray_item(vm, tinypy_tuple_get(args, 1U), &byte, out_error) == 0) return NULL;
    value = tinypy_tuple_get(args, 0U);
    size = (size_t)TINYPY_SIZE(value);
    assert(size != (size_t)PTRDIFF_MAX);
...
static tinypy_value_t *__tinypy_bytearray_extend_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_bytearray_method_arguments(vm, args, kwargs, 2U, 2U, out_error) == 0) return NULL;
    if (__tinypy_bytearray_collect(vm, tinypy_tuple_get(args, 1U), &extension, &extension_size, out_error) == 0) return NULL;
    value = tinypy_tuple_get(args, 0U);
    size = (size_t)TINYPY_SIZE(value);
    assert(size <= SIZE_MAX - extension_size);
    __tinypy_bytearray_reserve(value, size + extension_size);
    if (extension_size != 0U) (void)memcpy(TINYPY_BYTEARRAY_OBJECT(value)->bytes + size, extension, extension_size);
    TINYPY_SIZE(value) = (ptrdiff_t)(size + extension_size);
    if (extension != NULL) tinypy_internal_vm_deallocate(vm, extension, extension_size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    return tinypy_none_get(vm);
}
...
static int64_t __tinypy_bytearray_bound(tinypy_vm_t *vm, tinypy_value_t *value, size_t size, int64_t fallback, tinypy_error_t **out_error)
{
    ...
    if (bound < 0) bound = bound < -(int64_t)size ? 0 : bound + (int64_t)size;
    if ((uint64_t)bound > (uint64_t)size) bound = (int64_t)size;
    return bound;
}
...
static tinypy_value_t *__tinypy_bytearray_find_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_bytearray_method_arguments(vm, args, kwargs, 2U, 4U, out_error) == 0) return NULL;
    value = tinypy_tuple_get(args, 0U);
    if (tinypy_internal_bytes_view(tinypy_tuple_get(args, 1U), &needle, &needle_size) == 0) {
        tinypy_internal_make_vm_error(vm, TINYPY_ERROR_TYPE, "substring must support the buffer interface", out_error);
        return NULL;
    }
    size = (size_t)TINYPY_SIZE(value);
    start = __tinypy_bytearray_bound(vm, tinypy_tuple_size(args) >= 3U ? tinypy_tuple_get(args, 2U) : NULL, size, 0, out_error);
    if (start == INT64_MIN && out_error != NULL && *out_error != NULL) return NULL;
    stop = __tinypy_bytearray_bound(vm, tinypy_tuple_size(args) >= 4U ? tinypy_tuple_get(args, 3U) : NULL, size, (int64_t)size, out_error);
    if (stop == INT64_MIN && out_error != NULL && *out_error != NULL) return NULL;
    if (start <= stop && needle_size <= (size_t)(stop - start)) {
        if (needle_size == 0U) found = (ptrdiff_t)start;
        else {
            for (index = (size_t)start; index <= (size_t)stop - needle_size; index += 1U) {
