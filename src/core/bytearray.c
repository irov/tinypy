...
static void __tinypy_bytearray_reserve(tinypy_value_t *value, size_t minimum) {
    ...
    while (capacity < minimum) {
        ...
        TINYPY_ASSERT(grown > capacity);
        capacity = grown;
    }
    if (bytearray->bytes == NULL) {
        bytearray->bytes = (uint8_t *)tinypy_internal_vm_allocate(vm, capacity, (uint32_t)TINYPY_ALLOC_TAG_BYTEARRAY_DATA);
    }
    else {
        bytearray->bytes = (uint8_t *)tinypy_internal_vm_reallocate(vm, bytearray->bytes, bytearray->capacity, capacity, (uint32_t)TINYPY_ALLOC_TAG_BYTEARRAY_DATA);
    }
...
//////////////////////////////////////////////////////////////////////////
static int32_t __tinypy_bytearray_item(tinypy_vm_t *vm, tinypy_value_t *value, uint8_t *out_byte, tinypy_error_t **out_error) {
    int64_t integer;
...
//////////////////////////////////////////////////////////////////////////
static int32_t __tinypy_bytearray_collect(tinypy_vm_t *vm, tinypy_value_t *source, uint8_t **out_bytes, size_t *out_size, tinypy_error_t **out_error) {
    const uint8_t *view;
    size_t view_size;
    uint8_t *bytes = NULL;
    size_t size = 0U;
    ...
    *out_size = 0U;
    if (tinypy_internal_bytes_view(source, &view, &view_size) != 0) {
        if (view_size != 0U) {
            bytes = (uint8_t *)tinypy_internal_vm_allocate(vm, view_size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    ...
    for (;;) {
        ...
        if (size == capacity) {
            ...
            TINYPY_ASSERT(new_capacity > capacity);
            if (bytes == NULL) {
                bytes = (uint8_t *)tinypy_internal_vm_allocate(vm, new_capacity, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
            }
            else {
                bytes = (uint8_t *)tinypy_internal_vm_reallocate(vm, bytes, capacity, new_capacity, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
            }
    ...
    if (size != 0U && capacity != size) {
        bytes = (uint8_t *)tinypy_internal_vm_reallocate(vm, bytes, capacity, size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    }
    *out_bytes = bytes;
    *out_size = size;
    return INT32_C(1);
...
static void __tinypy_bytearray_replace_slice(tinypy_value_t *value, const tinypy_bytearray_slice_t *slice, const uint8_t *replacement, size_t replacement_size) {
    ...
    if (slice->step == 1) {
        ...
        TINYPY_ASSERT(old_size - slice->length <= SIZE_MAX - replacement_size);
    ...
    TINYPY_ASSERT(replacement_size == slice->length);
...
tinypy_value_t *tinypy_bytearray_from_bytes(tinypy_vm_t *vm, const void *bytes, size_t size) {
    ...
    if (size != 0U) {
        bytearray->bytes = (uint8_t *)tinypy_internal_vm_allocate(vm, size, (uint32_t)TINYPY_ALLOC_TAG_BYTEARRAY_DATA);
...
size_t tinypy_bytearray_size(const tinypy_value_t *value) {
    ...
    TINYPY_ASSERT(TINYPY_VALUE_KIND(value) == TINYPY_VALUE_BYTEARRAY);
...
tinypy_value_t *tinypy_internal_bytearray_create(tinypy_type_t *type, tinypy_value_t *args, tinypy_value_t *kwargs, tinypy_error_t **out_error) {
    ...
    if (__tinypy_bytearray_integer(source, &requested_size) != 0) {
        ...
        TINYPY_ASSERT((uint64_t)requested_size <= (uint64_t)SIZE_MAX);
...
tinypy_value_t *tinypy_internal_bytearray_get_item(tinypy_value_t *value, tinypy_value_t *key, tinypy_error_t **out_error) {
    ...
    if (TINYPY_VALUE_KIND(key) == TINYPY_VALUE_SLICE) {
        ...
        selected = (uint8_t *)tinypy_internal_vm_allocate(vm, slice.length, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
...
tinypy_value_t *tinypy_internal_bytearray_repr(tinypy_value_t *value, tinypy_error_t **out_error) {
    ...
    TINYPY_ASSERT(quoted_size <= SIZE_MAX - 12U);
    bytes = (uint8_t *)tinypy_internal_vm_allocate(vm, quoted_size + 12U, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
...
static tinypy_value_t *__tinypy_bytearray_add_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    bytes = (uint8_t *)tinypy_internal_vm_allocate(vm, left_size + right_size, (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    if (left_size != 0U) {
...
static tinypy_value_t *__tinypy_bytearray_extend_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    tinypy_value_t *value = TINYPY_TUPLE_GET(args, 0U);
    size = TINYPY_SIZED_SIZE(value);
    TINYPY_ASSERT(size <= SIZE_MAX - extension_size);
