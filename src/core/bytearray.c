...
static int32_t __tinypy_bytearray_slice(tinypy_vm_t *vm, tinypy_value_t *slice_value, size_t size, tinypy_bytearray_slice_t *out_slice, tinypy_error_t **out_error) {
    ...
    if (tinypy_internal_value_kind(step_value) == TINYPY_VALUE_NONE) {
    ...
    if (tinypy_internal_value_kind(start_value) == TINYPY_VALUE_NONE) {
    ...
    if (tinypy_internal_value_kind(stop_value) == TINYPY_VALUE_NONE) {
...
tinypy_value_t *tinypy_internal_bytearray_get_item(tinypy_value_t *value, tinypy_value_t *key, tinypy_error_t **out_error) {
    ...
    if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) {
        ...
        for (selected_index = 0U; selected_index < slice.length; selected_index += 1U) {
...
int32_t tinypy_internal_bytearray_set_item(tinypy_value_t *value, tinypy_value_t *key, tinypy_value_t *item, tinypy_error_t **out_error) {
    ...
    if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) {
        ...
        if (slice.step > 0) {
            ...
            for (deletion_index = 0U; deletion_index < slice.length; deletion_index += 1U) {
...
tinypy_value_t *tinypy_internal_bytearray_string(tinypy_value_t *value, tinypy_error_t **out_error) {
    tinypy_internal_clear_error(out_error);
    tinypy_vm_t *vm = tinypy_internal_value_vm(value);
    return tinypy_string_from_bytes(vm, TINYPY_BYTEARRAY_OBJECT(value)->bytes, (size_t)TINYPY_SIZE(value));
}
