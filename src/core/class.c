...
tinypy_value_t *tinypy_internal_class_lookup(tinypy_value_t *class_value, const char *name, size_t name_size)
{
    ...
    for (index = 0U; index < tinypy_tuple_size(class_object->bases); index += 1U) {
        attribute = tinypy_internal_class_lookup(tinypy_tuple_get(class_object->bases, index), name, name_size);
        if (attribute != NULL) return attribute;
    }
    return NULL;
}
...
int32_t tinypy_class_is_subclass(const tinypy_value_t *class_value, const tinypy_value_t *candidate_base)
{
    ...
    if (class_value == candidate_base) return INT32_C(1);
    for (index = 0U; index < tinypy_tuple_size(class_object->bases); index += 1U) {
        if (tinypy_class_is_subclass(tinypy_tuple_get(class_object->bases, index), candidate_base) != 0) return INT32_C(1);
    }
    return INT32_C(0);
}
...
static tinypy_value_t *__tinypy_class_bind(tinypy_value_t *class_value, tinypy_value_t *attribute, tinypy_value_t *instance)
{
    ...
    if (kind == TINYPY_VALUE_FUNCTION) return tinypy_method_new(attribute, instance, class_value);
    ...
    if (kind == TINYPY_VALUE_CLASS_METHOD) return tinypy_method_new(tinypy_class_method_callable(attribute), class_value, class_value);
...
static tinypy_value_t *__tinypy_class_special_attribute(tinypy_value_t *class_value, tinypy_value_t *name)
{
    ...
    if (__tinypy_class_name_equal(name, "__name__", 8U) != 0) result = class_object->name;
    else if (__tinypy_class_name_equal(name, "__bases__", 9U) != 0) result = class_object->bases;
    else if (__tinypy_class_name_equal(name, "__dict__", 8U) != 0) result = class_object->dict;
    if (result != NULL) tinypy_retain(result);
    return result;
}
...
tinypy_value_t *tinypy_internal_old_instance_get_attribute(tinypy_value_t *instance_value, tinypy_value_t *name, tinypy_error_t **out_error)
{
    ...
    if (result != NULL || __tinypy_class_name_equal(name, "__getattr__", 11U) != 0) return result;
    {
        tinypy_value_t *hook_attribute = tinypy_internal_class_lookup(instance->class_object, "__getattr__", 11U);
        tinypy_value_t *hook;
        tinypy_value_t *args;

        if (hook_attribute == NULL) return NULL;
...
{
    return __tinypy_class_delete_from_dict(__tinypy_class_validate(value)->dict, name, name_size, out_error);
}
...
{
    return __tinypy_class_delete_from_dict(__tinypy_old_instance_validate(value)->dict, name, name_size, out_error);
}
...
tinypy_value_t *tinypy_class_name(const tinypy_value_t *class_value) { return __tinypy_class_validate(class_value)->name; }
tinypy_value_t *tinypy_class_bases(const tinypy_value_t *class_value) { return __tinypy_class_validate(class_value)->bases; }
tinypy_value_t *tinypy_class_dict(const tinypy_value_t *class_value) { return __tinypy_class_validate(class_value)->dict; }
tinypy_value_t *tinypy_old_instance_class(const tinypy_value_t *instance) { return __tinypy_old_instance_validate(instance)->class_object; }
tinypy_value_t *tinypy_old_instance_dict(const tinypy_value_t *instance) { return __tinypy_old_instance_validate(instance)->dict; }
