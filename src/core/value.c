...
tinypy_type_t *tinypy_internal_type_for_kind(
    tinypy_vm_t *vm,
    tinypy_value_type_e kind)
{
    switch (kind) {
    case TINYPY_VALUE_NONE: return &vm->none_type;
    case TINYPY_VALUE_NOT_IMPLEMENTED: return &vm->not_implemented_type;
    case TINYPY_VALUE_BOOL: return &vm->bool_type;
    case TINYPY_VALUE_INTEGER: return &vm->integer_type;
    case TINYPY_VALUE_STRING: return &vm->string_type;
    case TINYPY_VALUE_UNICODE: return &vm->unicode_type;
    case TINYPY_VALUE_LONG: return &vm->long_type;
    case TINYPY_VALUE_FLOAT: return &vm->float_type;
    case TINYPY_VALUE_COMPLEX: return &vm->complex_type;
    case TINYPY_VALUE_TUPLE: return &vm->tuple_type;
    case TINYPY_VALUE_LIST: return &vm->list_type;
    case TINYPY_VALUE_DICT: return &vm->dict_type;
    case TINYPY_VALUE_SET: return &vm->set_type;
    case TINYPY_VALUE_FROZENSET: return &vm->frozenset_type;
    case TINYPY_VALUE_OUTPUT_STREAM: return &vm->output_stream_type;
    case TINYPY_VALUE_CODE: return &vm->code_type;
    case TINYPY_VALUE_FRAME: return &vm->frame_type;
    case TINYPY_VALUE_FUNCTION: return &vm->function_type;
    case TINYPY_VALUE_ITERATOR: return &vm->iterator_type;
    case TINYPY_VALUE_METHOD: return &vm->method_type;
    case TINYPY_VALUE_CELL: return &vm->cell_type;
    case TINYPY_VALUE_SLICE: return &vm->slice_type;
    case TINYPY_VALUE_MODULE: return &vm->module_type;
    case TINYPY_VALUE_NATIVE_FUNCTION: return &vm->native_function_type;
    case TINYPY_VALUE_STATIC_METHOD: return &vm->static_method_type;
    case TINYPY_VALUE_CLASS_METHOD: return &vm->class_method_type;
    case TINYPY_VALUE_PROPERTY: return &vm->property_type;
    case TINYPY_VALUE_SUPER: return &vm->super_type;
    case TINYPY_VALUE_TRACEBACK: return &vm->traceback_type;
    case TINYPY_VALUE_GENERATOR: return &vm->generator_type;
    case TINYPY_VALUE_XRANGE: return &vm->xrange_type;
    case TINYPY_VALUE_ENUMERATE: return &vm->enumerate_type;
    case TINYPY_VALUE_REVERSED: return &vm->reversed_type;
    case TINYPY_VALUE_BUFFER: return &vm->buffer_type;
    case TINYPY_VALUE_BYTEARRAY: return &vm->bytearray_type;
    case TINYPY_VALUE_WEAKREF: return &vm->weakref_type;
    case TINYPY_VALUE_DICT_KEYS: return &vm->dict_keys_type;
    case TINYPY_VALUE_DICT_VALUES: return &vm->dict_values_type;
    case TINYPY_VALUE_DICT_ITEMS: return &vm->dict_items_type;
    case TINYPY_VALUE_ELLIPSIS: return &vm->ellipsis_type;
    case TINYPY_VALUE_FILE: return &vm->file_type;
    case TINYPY_VALUE_GETSET_DESCRIPTOR: return &vm->getset_descriptor_type;
    case TINYPY_VALUE_MEMBER_DESCRIPTOR: return &vm->member_descriptor_type;
    case TINYPY_VALUE_CLASS: return &vm->class_type;
    case TINYPY_VALUE_OLD_INSTANCE: return &vm->old_instance_type;
    case TINYPY_VALUE_PARTIAL: return &vm->partial_type;
    case TINYPY_VALUE_SRE_PATTERN: return &vm->sre_pattern_type;
    case TINYPY_VALUE_SRE_MATCH: return &vm->sre_match_type;
    default:
...
tinypy_value_type_e tinypy_internal_value_kind(const tinypy_value_t *value)
{
    ...
    type = value->type;
    vm = type->vm;
    if (type == &vm->none_type) return TINYPY_VALUE_NONE;
    if (type == &vm->not_implemented_type) return TINYPY_VALUE_NOT_IMPLEMENTED;
    if (type == &vm->bool_type) return TINYPY_VALUE_BOOL;
    if (type == &vm->integer_type) return TINYPY_VALUE_INTEGER;
    if (type == &vm->string_type) return TINYPY_VALUE_STRING;
    if (type == &vm->unicode_type) return TINYPY_VALUE_UNICODE;
    if (type == &vm->long_type) return TINYPY_VALUE_LONG;
    if (type == &vm->float_type) return TINYPY_VALUE_FLOAT;
    if (type == &vm->complex_type) return TINYPY_VALUE_COMPLEX;
    if (type == &vm->tuple_type) return TINYPY_VALUE_TUPLE;
    if (type == &vm->list_type) return TINYPY_VALUE_LIST;
    if (type == &vm->dict_type) return TINYPY_VALUE_DICT;
    if (type == &vm->set_type) return TINYPY_VALUE_SET;
    if (type == &vm->frozenset_type) return TINYPY_VALUE_FROZENSET;
    if (type == &vm->output_stream_type) return TINYPY_VALUE_OUTPUT_STREAM;
    if (type == &vm->code_type) return TINYPY_VALUE_CODE;
    if (type == &vm->frame_type) return TINYPY_VALUE_FRAME;
    if (type == &vm->function_type) return TINYPY_VALUE_FUNCTION;
    if (type == &vm->iterator_type) return TINYPY_VALUE_ITERATOR;
    if (type == &vm->method_type) return TINYPY_VALUE_METHOD;
    if (type == &vm->cell_type) return TINYPY_VALUE_CELL;
    if (type == &vm->slice_type) return TINYPY_VALUE_SLICE;
    if (type == &vm->module_type) return TINYPY_VALUE_MODULE;
    if (type == &vm->native_function_type) return TINYPY_VALUE_NATIVE_FUNCTION;
    if (type == &vm->static_method_type) return TINYPY_VALUE_STATIC_METHOD;
    if (type == &vm->class_method_type) return TINYPY_VALUE_CLASS_METHOD;
    if (type == &vm->property_type) return TINYPY_VALUE_PROPERTY;
    if (type == &vm->super_type) return TINYPY_VALUE_SUPER;
    if (type == &vm->traceback_type) return TINYPY_VALUE_TRACEBACK;
    if (type == &vm->generator_type) return TINYPY_VALUE_GENERATOR;
    if (type == &vm->xrange_type) return TINYPY_VALUE_XRANGE;
    if (type == &vm->enumerate_type) return TINYPY_VALUE_ENUMERATE;
    if (type == &vm->reversed_type) return TINYPY_VALUE_REVERSED;
    if (type == &vm->buffer_type) return TINYPY_VALUE_BUFFER;
    if (type == &vm->bytearray_type) return TINYPY_VALUE_BYTEARRAY;
    if (type == &vm->weakref_type) return TINYPY_VALUE_WEAKREF;
    if (type == &vm->dict_keys_type) return TINYPY_VALUE_DICT_KEYS;
    if (type == &vm->dict_values_type) return TINYPY_VALUE_DICT_VALUES;
    if (type == &vm->dict_items_type) return TINYPY_VALUE_DICT_ITEMS;
    if (type == &vm->ellipsis_type) return TINYPY_VALUE_ELLIPSIS;
    if (type == &vm->file_type) return TINYPY_VALUE_FILE;
    if (type == &vm->getset_descriptor_type) return TINYPY_VALUE_GETSET_DESCRIPTOR;
    if (type == &vm->member_descriptor_type) return TINYPY_VALUE_MEMBER_DESCRIPTOR;
    if (type == &vm->class_type) return TINYPY_VALUE_CLASS;
    if (type == &vm->old_instance_type) return TINYPY_VALUE_OLD_INSTANCE;
    if (type == &vm->partial_type) return TINYPY_VALUE_PARTIAL;
    if (type == &vm->sre_pattern_type) return TINYPY_VALUE_SRE_PATTERN;
    if (type == &vm->sre_match_type) return TINYPY_VALUE_SRE_MATCH;
    if ((type->flags & TINYPY_TYPE_FLAG_HEAP) != 0U && type->layout_kind != TINYPY_VALUE_INVALID) return type->layout_kind;
...
tinypy_value_t *tinypy_internal_value_allocate(
    tinypy_vm_t *vm,
...
tinypy_value_t *tinypy_internal_object_allocate(
    tinypy_vm_t *vm,
...
size_t tinypy_internal_value_allocation_size(const tinypy_value_t *value)
{
    ...
    case TINYPY_VALUE_STRING:
        return offsetof(tinypy_string_object_t, bytes) +
            (size_t)TINYPY_SIZE(value) + 1U;
    case TINYPY_VALUE_UNICODE:
        return offsetof(tinypy_unicode_object_t, utf8) +
            TINYPY_UNICODE_OBJECT(value)->byte_size + 1U;
    case TINYPY_VALUE_LONG:
        return offsetof(tinypy_long_object_t, digits) +
            TINYPY_LONG_DIGIT_COUNT(value) * sizeof(uint16_t);
    ...
    case TINYPY_VALUE_FRAME:
        return offsetof(tinypy_frame_object_t, locals_plus) +
            (size_t)TINYPY_SIZE(value) * sizeof(tinypy_value_t *);
    default:
        return value->type->basic_size;
    }
}
...
int tinypy_internal_value_is_vm_embedded(
    ...
    const tinypy_value_t *value)
{
    ...
    if (value == &vm->none_object.base ||
        value == &vm->not_implemented_object.base ||
        value == &vm->ellipsis_object.base ||
        value == &vm->false_object.base ||
        value == &vm->true_object.base ||
        value == &vm->float_zero_object.base ||
        value == &vm->empty_string_object.base.base ||
        value == &vm->empty_tuple_object.base.base) {
        return 1;
    }
    ...
    for (index = 0U; index < TINYPY_BUILTIN_TYPE_COUNT; index += 1U) {
        if (value == &types[index]->base.base ||
            value == &vm->builtin_type_dicts[index].base) {
            return 1;
        }
    }
...
static int32_t __tinypy_internal_value_finalize(tinypy_value_t *value)
{
    ...
    if (value->type->has_finalizer == 0 && (tinypy_internal_value_kind(value) != TINYPY_VALUE_OLD_INSTANCE || tinypy_internal_old_instance_has_special(value, "__del__", 7U) == 0)) return INT32_C(0);
    value->ref = 1;
...
void tinypy_release(tinypy_value_t *value)
{
    ...
    if (__tinypy_internal_value_finalize(value) != 0) return;
...
static size_t __tinypy_internal_text_allocation_size(
    tinypy_value_type_e type,
    size_t byte_size)
{
...
static tinypy_value_t *__tinypy_internal_text_from_bytes(
    tinypy_vm_t *vm,
...
const unsigned char *tinypy_internal_text_bytes(const tinypy_value_t *value)
{
    assert(tinypy_internal_value_kind(value) == TINYPY_VALUE_STRING ||
        tinypy_internal_value_kind(value) == TINYPY_VALUE_UNICODE);
...
static size_t __tinypy_internal_utf8_code_point_count(
    const unsigned char *bytes,
    size_t size)
{
    size_t offset = 0U;
...
int tinypy_internal_value_belongs_to(
    const tinypy_vm_t *vm,
    const tinypy_value_t *value)
{
    return value != NULL &&
        tinypy_internal_value_vm(value) == vm;
}
...
tinypy_value_t *tinypy_integer_from_i64(tinypy_vm_t *vm, int64_t value)
{
    ...
    assert(tinypy_internal_vm_valid(vm));
    if (value >= TINYPY_INTEGER_CONSTANT_MIN &&
        value <= TINYPY_INTEGER_CONSTANT_MAX) {
...
tinypy_value_t *tinypy_string_from_bytes(
    tinypy_vm_t *vm,
    const void *bytes,
    size_t size)
{
...
const char *tinypy_unicode_utf8_view(
    const tinypy_value_t *value,
    size_t *out_size,
...
const tinypy_type_t *tinypy_type_metaclass(
    const tinypy_type_t *type)
{
    assert(type != NULL);
    assert(tinypy_internal_vm_valid(type->vm));
...
const tinypy_type_t *tinypy_type_base(
    const tinypy_type_t *type)
{
    assert(type != NULL);
    assert(tinypy_internal_vm_valid(type->vm));
...
const tinypy_value_t *tinypy_type_dict(
    const tinypy_type_t *type)
{
    assert(type != NULL);
    assert(tinypy_internal_vm_valid(type->vm));
...
void tinypy_internal_type_release_references(
    tinypy_value_t *value,
