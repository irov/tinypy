...
tinypy_value_t *tinypy_frame_new(tinypy_value_t *code, tinypy_value_t *globals, tinypy_value_t *locals)
{
    ...
    cell_count = tinypy_tuple_size(tinypy_code_cellvars(code));
    free_count = tinypy_tuple_size(tinypy_code_freevars(code));
...
void tinypy_internal_frame_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    size_t local_slot_count = (size_t)tinypy_code_local_count(frame->code) + tinypy_tuple_size(tinypy_code_cellvars(frame->code)) + tinypy_tuple_size(tinypy_code_freevars(frame->code));
    size_t index;
    ...
    if (frame->previous_handled_type != NULL) visit(frame->previous_handled_type, user_data);
    if (frame->previous_handled_value != NULL) visit(frame->previous_handled_value, user_data);
    if (frame->previous_handled_traceback != NULL) visit(frame->previous_handled_traceback, user_data);
...
tinypy_value_t *tinypy_frame_back(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->back; }
tinypy_value_t *tinypy_frame_code(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->code; }
tinypy_value_t *tinypy_frame_builtins(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->builtins; }
tinypy_value_t *tinypy_frame_globals(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->globals; }
tinypy_value_t *tinypy_frame_locals(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->locals; }
int32_t tinypy_frame_last_instruction(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->last_instruction; }
int32_t tinypy_frame_line_number(const tinypy_value_t *frame) { return __tinypy_internal_frame_validate(frame)->line_number; }
size_t tinypy_frame_stack_depth(const tinypy_value_t *frame) { tinypy_frame_object_t *object = __tinypy_internal_frame_validate(frame); return (size_t)(object->stack_top - object->value_stack); }
