...
{
    tinypy_value_type_e kind = tinypy_internal_value_kind(key);
    ...
    if (kind == TINYPY_VALUE_LONG) {
        ...
        if (TINYPY_LONG_SIGN(key) >= 0) {
            ...
            *out_index = (int64_t)magnitude;
            ...
            if (magnitude > (uint64_t)INT64_MAX + UINT64_C(1)) return 0;
...
static int __tinypy_item_slice_indices(tinypy_vm_t *vm, tinypy_value_t *slice_value, size_t size, tinypy_item_slice_indices_t *out_indices, tinypy_error_t **out_error)
{
    ...
    if (tinypy_internal_value_kind(slice->step) == TINYPY_VALUE_NONE) {
        out_indices->step = 1;
    } else if (__tinypy_item_slice_index_value(vm, slice->step, &out_indices->step, out_error) == 0) {
        return 0;
    }
    if (out_indices->step == INT64_MIN) out_indices->step = -INT64_MAX;
    ...
    if (tinypy_internal_value_kind(slice->start) == TINYPY_VALUE_NONE) {
        out_indices->start = out_indices->step < 0 ? upper : lower;
    } else {
        if (__tinypy_item_slice_index_value(vm, slice->start, &out_indices->start, out_error) == 0) return 0;
        if (out_indices->start < 0) out_indices->start += sequence_size;
        if (out_indices->start < lower) out_indices->start = lower;
        if (out_indices->start > upper) out_indices->start = upper;
    }
    if (tinypy_internal_value_kind(slice->stop) == TINYPY_VALUE_NONE) {
        out_indices->stop = out_indices->step < 0 ? lower : upper;
    } else {
        if (__tinypy_item_slice_index_value(vm, slice->stop, &out_indices->stop, out_error) == 0) return 0;
        if (out_indices->stop < 0) out_indices->stop += sequence_size;
        if (out_indices->stop < lower) out_indices->stop = lower;
        if (out_indices->stop > upper) out_indices->stop = upper;
    }
...
{
    tinypy_vm_t *vm = tinypy_internal_value_vm(container);
...
static tinypy_value_t *__tinypy_item_unicode_get(tinypy_value_t *container, size_t scalar_index)
{
    ...
    return tinypy_unicode_from_utf8(tinypy_internal_value_vm(container), utf8 + byte_index, scalar_size);
}
...
static tinypy_value_t *__tinypy_item_sequence_slice(tinypy_value_t *container, const tinypy_item_slice_indices_t *indices)
{
    ...
    if (indices->length != 0U) {
        ...
        for (index = 0U; index < indices->length; index += 1U) {
            items[index] = tinypy_internal_value_kind(container) == TINYPY_VALUE_TUPLE ? tinypy_tuple_get(container, (size_t)source_index) : tinypy_list_get(container, (size_t)source_index);
            source_index += indices->step;
        }
    }
    result = tinypy_internal_value_kind(container) == TINYPY_VALUE_TUPLE ? tinypy_tuple_from_items(vm, items, indices->length) : tinypy_list_from_items(vm, items, indices->length);
    if (items != NULL) tinypy_internal_vm_deallocate(vm, items, indices->length * sizeof(*items), (uint32_t)TINYPY_ALLOC_TAG_TEMPORARY);
    return result;
}
...
static tinypy_value_t *__tinypy_item_string_slice(tinypy_value_t *container, const tinypy_item_slice_indices_t *indices)
{
    ...
    if (indices->length == 0U) return tinypy_string_from_bytes(vm, NULL, 0U);
    if (indices->step == 1) return tinypy_string_from_bytes(vm, bytes + (size_t)indices->start, indices->length);
...
static tinypy_value_t *__tinypy_item_unicode_slice(tinypy_value_t *container, const tinypy_item_slice_indices_t *indices)
{
    ...
    utf8 = tinypy_unicode_utf8_view(container, &byte_size, &code_point_count);
    if (indices->length == 0U) return tinypy_unicode_from_utf8(vm, NULL, 0U);
    ...
    for (scalar_index = 0U; scalar_index < code_point_count; scalar_index += 1U) {
        unsigned char lead = (unsigned char)utf8[byte_index];
...
static int32_t __tinypy_item_list_set_slice(tinypy_value_t *list, tinypy_value_t *slice, tinypy_value_t *value, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_item_slice_indices(vm, slice, tinypy_list_size(list), &indices, out_error) == 0) return 0;
    ...
    if (indices.step == 1) {
        for (index = 0U; index < indices.length; index += 1U) tinypy_list_delete(list, (size_t)indices.start);
        for (index = 0U; index < replacement_size; index += 1U) tinypy_list_insert(list, (size_t)indices.start + index, tinypy_list_get(replacement, index));
    } else {
        target_index = indices.start;
        for (index = 0U; index < indices.length; index += 1U) {
            tinypy_list_set(list, (size_t)target_index, tinypy_list_get(replacement, index));
...
static int32_t __tinypy_item_list_delete_slice(tinypy_value_t *list, tinypy_value_t *slice, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_item_slice_indices(tinypy_internal_value_vm(list), slice, tinypy_list_size(list), &indices, out_error) == 0) return 0;
    if (indices.step > 0) {
        for (index = indices.length; index != 0U; index -= 1U) tinypy_list_delete(list, (size_t)(indices.start + (int64_t)(index - 1U) * indices.step));
    } else {
        for (index = 0U; index < indices.length; index += 1U) tinypy_list_delete(list, (size_t)(indices.start + (int64_t)index * indices.step));
    }
    return 1;
}
...
tinypy_value_t *tinypy_get_item(tinypy_value_t *container, tinypy_value_t *key, tinypy_error_t **out_error)
{
    ...
    if (container->type->mapping_slots != NULL && container->type->mapping_slots->get_item != NULL) return container->type->mapping_slots->get_item(container, key, out_error);
    ...
    if (kind == TINYPY_VALUE_DICT) {
        ...
        item = tinypy_dict_get(container, key);
        tinypy_retain(item);
        return item;
    }
    if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) {
        ...
        if (kind == TINYPY_VALUE_TUPLE) size = tinypy_tuple_size(container);
        else if (kind == TINYPY_VALUE_LIST) size = tinypy_list_size(container);
        else if (kind == TINYPY_VALUE_STRING) (void)tinypy_string_view(container, &size);
        else if (kind == TINYPY_VALUE_UNICODE) {
            size_t byte_size;
            (void)tinypy_unicode_utf8_view(container, &byte_size, &size);
        } else {
            if (tinypy_internal_object_has_special(container, "__getitem__", 11U) != 0) return __tinypy_item_call_method(container, "__getitem__", 11U, &key, 1U, out_error);
            tinypy_internal_make_vm_error(vm, TINYPY_ERROR_TYPE, "object does not support slicing", out_error);
            return NULL;
        }
        if (__tinypy_item_slice_indices(vm, key, size, &indices, out_error) == 0) return NULL;
        if (kind == TINYPY_VALUE_TUPLE || kind == TINYPY_VALUE_LIST) return __tinypy_item_sequence_slice(container, &indices);
        if (kind == TINYPY_VALUE_STRING) return __tinypy_item_string_slice(container, &indices);
        return __tinypy_item_unicode_slice(container, &indices);
    }
    if (kind == TINYPY_VALUE_TUPLE) {
        if (__tinypy_item_normalize_index(vm, key, tinypy_tuple_size(container), &index, out_error) == 0) return NULL;
        item = tinypy_tuple_get(container, index);
        tinypy_retain(item);
        return item;
    }
    if (kind == TINYPY_VALUE_LIST) {
        if (__tinypy_item_normalize_index(vm, key, tinypy_list_size(container), &index, out_error) == 0) return NULL;
    ...
    if (kind == TINYPY_VALUE_STRING) {
        ...
        if (__tinypy_item_normalize_index(vm, key, size, &index, out_error) == 0) return NULL;
    ...
    if (kind == TINYPY_VALUE_UNICODE) {
        ...
        if (__tinypy_item_normalize_index(vm, key, code_point_count, &index, out_error) == 0) return NULL;
    ...
    if (kind == TINYPY_VALUE_XRANGE) {
        ...
        if (__tinypy_item_normalize_index(vm, key, range->length, &index, out_error) == 0) return NULL;
...
int32_t tinypy_set_item(tinypy_value_t *container, tinypy_value_t *key, tinypy_value_t *value, tinypy_error_t **out_error)
{
    ...
    tinypy_internal_clear_error(out_error);
    if (container->type->mapping_slots != NULL && container->type->mapping_slots->set_item != NULL) return container->type->mapping_slots->set_item(container, key, value, out_error);
    kind = tinypy_internal_value_kind(container);
    if (kind == TINYPY_VALUE_DICT) {
    ...
    if (kind == TINYPY_VALUE_LIST) {
        if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) return __tinypy_item_list_set_slice(container, key, value, out_error);
        if (__tinypy_item_normalize_index(vm, key, tinypy_list_size(container), &index, out_error) == 0) return 0;
...
int32_t tinypy_delete_item(tinypy_value_t *container, tinypy_value_t *key, tinypy_error_t **out_error)
{
    ...
    assert(tinypy_internal_value_belongs_to(vm, key));
    tinypy_internal_clear_error(out_error);
    if (container->type->mapping_slots != NULL && container->type->mapping_slots->set_item != NULL) return container->type->mapping_slots->set_item(container, key, NULL, out_error);
    kind = tinypy_internal_value_kind(container);
    if (kind == TINYPY_VALUE_DICT) {
        if (tinypy_dict_contains(container, key) == 0) {
            tinypy_internal_make_vm_error(vm, TINYPY_ERROR_KEY, "dictionary key is absent", out_error);
    ...
    if (kind == TINYPY_VALUE_LIST) {
        if (tinypy_internal_value_kind(key) == TINYPY_VALUE_SLICE) return __tinypy_item_list_delete_slice(container, key, out_error);
        if (__tinypy_item_normalize_index(vm, key, tinypy_list_size(container), &index, out_error) == 0) return 0;
