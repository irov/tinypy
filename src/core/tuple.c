...
void tinypy_internal_tuple_release_references(
...
void tinypy_internal_tuple_subclass_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    for (index = 0U; index < (size_t)TINYPY_SIZE(value); index += 1U) visit(items[index], user_data);
...
tinypy_value_t *tinypy_internal_tuple_subclass_from_items(tinypy_type_t *type, tinypy_value_t *const *items, size_t size)
{
    ...
    if (size != 0U) tuple->items = (tinypy_value_t **)tinypy_internal_vm_allocate(vm, size * sizeof(*tuple->items), (uint32_t)TINYPY_ALLOC_TAG_TUPLE_ITEMS);
    for (index = 0U; index < size; index += 1U) {
...
tinypy_value_t *tinypy_tuple_from_items(
    tinypy_vm_t *vm,
    tinypy_value_t *const *items,
    size_t size)
{
...
tinypy_value_t *tinypy_tuple_get(
