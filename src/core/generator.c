...
void tinypy_internal_generator_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    if (generator->frame != NULL) visit(generator->frame, user_data);
    if (generator->handled_type != NULL) visit(generator->handled_type, user_data);
    if (generator->handled_value != NULL) visit(generator->handled_value, user_data);
    if (generator->handled_traceback != NULL) visit(generator->handled_traceback, user_data);
}
...
tinypy_value_t *tinypy_generator_send(tinypy_value_t *generator_value, tinypy_value_t *value, tinypy_error_t **out_error)
{
    ...
    if (generator->finished != 0) return NULL;
...
int32_t tinypy_generator_close(tinypy_value_t *generator_value, tinypy_error_t **out_error)
{
    ...
    tinypy_internal_clear_error(out_error);
    if (generator->finished != 0) return INT32_C(1);
    ...
    if (exception == NULL) return INT32_C(0);
...
{
    tinypy_value_t *none = tinypy_none_get(tinypy_internal_value_vm(value));
...
static tinypy_value_t *__tinypy_generator_next_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_generator_method_arguments(vm, args, kwargs, 1U, out_error) == 0) return NULL;
...
static tinypy_value_t *__tinypy_generator_send_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_generator_method_arguments(vm, args, kwargs, 2U, out_error) == 0) return NULL;
    result = tinypy_generator_send(tinypy_tuple_get(args, 0U), tinypy_tuple_get(args, 1U), out_error);
    if (result != NULL || vm->raised_value != NULL) return result;
    tinypy_internal_exception_raise_stop_iteration(vm, out_error);
    return NULL;
}
...
static tinypy_value_t *__tinypy_generator_throw_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    if (count == 4U && tinypy_internal_value_kind(tinypy_tuple_get(args, 3U)) != TINYPY_VALUE_NONE) {
    ...
    if (tinypy_internal_value_kind(exception_argument) == TINYPY_VALUE_TYPE && tinypy_type_is_subtype((tinypy_type_t *)exception_argument, vm->exception_types[TINYPY_EXCEPTION_BASE]) != 0) {
        if (count >= 3U && tinypy_type_is_subtype(tinypy_tuple_get(args, 2U)->type, (tinypy_type_t *)exception_argument) != 0) {
            exception = tinypy_tuple_get(args, 2U);
            tinypy_retain(exception);
        } else {
            tinypy_value_t *exception_args;

            if (count >= 3U) exception_args = tinypy_tuple_from_items(vm, &tinypy_internal_tuple_items(args)[2], 1U);
            else exception_args = tinypy_tuple_from_items(vm, NULL, 0U);
...
static tinypy_value_t *__tinypy_generator_close_method(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error)
{
    ...
    (void)user_data;
    if (__tinypy_generator_method_arguments(vm, args, kwargs, 1U, out_error) == 0) return NULL;
    if (tinypy_generator_close(tinypy_tuple_get(args, 0U), out_error) == 0) return NULL;
...
tinypy_value_t *tinypy_generator_frame(const tinypy_value_t *generator) { return __tinypy_generator_validate(generator)->frame; }
int32_t tinypy_generator_finished(const tinypy_value_t *generator) { return __tinypy_generator_validate(generator)->finished != 0 ? 1 : 0; }
