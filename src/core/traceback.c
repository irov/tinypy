...
tinypy_value_t *tinypy_internal_traceback_new(tinypy_value_t *frame, tinypy_value_t *next)
{
    ...
    if (next != NULL) tinypy_retain(next);
    tinypy_retain(frame);
...
void tinypy_internal_traceback_release_references(tinypy_value_t *value, tinypy_release_callback_t visit, void *user_data)
{
    ...
    if (traceback->next != NULL) visit(traceback->next, user_data);
...
tinypy_value_t *tinypy_traceback_next(const tinypy_value_t *traceback) { return __tinypy_traceback_validate(traceback)->next; }
tinypy_value_t *tinypy_traceback_frame(const tinypy_value_t *traceback) { return __tinypy_traceback_validate(traceback)->frame; }
int32_t tinypy_traceback_last_instruction(const tinypy_value_t *traceback) { return __tinypy_traceback_validate(traceback)->last_instruction; }
int32_t tinypy_traceback_line_number(const tinypy_value_t *traceback) { return __tinypy_traceback_validate(traceback)->line_number; }
