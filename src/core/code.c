...
static int32_t __tinypy_internal_code_all_name_chars(const tinypy_value_t *value)
{
    ...
    for (index = 0U; index < size; index += 1U) {
        ...
        if ((byte >= '0' && byte <= '9') || (byte >= 'A' && byte <= 'Z') || byte == '_' || (byte >= 'a' && byte <= 'z')) continue;
        return INT32_C(0);
    }
    return INT32_C(1);
}
...
static void __tinypy_internal_code_intern_constants(tinypy_value_t *value)
{
    ...
    if (type == TINYPY_VALUE_STRING) {
        if (__tinypy_internal_code_all_name_chars(value) != 0) tinypy_internal_string_set_interned(value, 1);
        return;
    }
    if (type == TINYPY_VALUE_TUPLE) {
        for (index = 0U; index < tinypy_tuple_size(value); index += 1U) __tinypy_internal_code_intern_constants(tinypy_tuple_get(value, index));
        return;
    }
    if (type == TINYPY_VALUE_FROZENSET) {
        ...
        for (index = 0U; index <= dict->mask; index += 1U) {
            if (dict->table[index].state == TINYPY_DICT_ENTRY_ACTIVE) __tinypy_internal_code_intern_constants(dict->table[index].key);
        }
    }
}
...
int32_t tinypy_code_local_count(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->local_count; }
int32_t tinypy_code_stack_size(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->stack_size; }
int32_t tinypy_code_flags(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->flags; }
tinypy_value_t *tinypy_code_bytecode(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->bytecode; }
tinypy_value_t *tinypy_code_consts(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->consts; }
tinypy_value_t *tinypy_code_names(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->names; }
tinypy_value_t *tinypy_code_varnames(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->varnames; }
tinypy_value_t *tinypy_code_freevars(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->freevars; }
tinypy_value_t *tinypy_code_cellvars(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->cellvars; }
tinypy_value_t *tinypy_code_filename(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->filename; }
tinypy_value_t *tinypy_code_name(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->name; }
int32_t tinypy_code_first_line_number(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->first_line_number; }
tinypy_value_t *tinypy_code_lnotab(const tinypy_value_t *code) { return __tinypy_internal_code_validate(code)->lnotab; }
