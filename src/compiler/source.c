...
static int32_t __tinypy_compiler_is_utf8_cookie(const unsigned char *name, size_t size)
{
    char normalized[16];
    size_t index;
    size_t output_size = 0U;

    if (size >= sizeof(normalized)) return 0;
...
static int32_t __tinypy_compiler_encoding_cookie(const unsigned char *source, size_t size, const unsigned char **out_name, size_t *out_size, int32_t *out_line)
{
    ...
    while (line < 2U && position < size) {
        ...
        while (line_end < size && source[line_end] != '\n' && source[line_end] != '\r') line_end += 1U;
        marker = position;
        while (marker < line_end && (source[marker] == ' ' || source[marker] == '\t' || source[marker] == '\f')) marker += 1U;
        if (marker < line_end && source[marker] != '#') return 0;
        for (; marker + 6U <= line_end; marker += 1U) {
            size_t name_start;
            size_t name_end;

            if (source[marker] != 'c' || source[marker + 1U] != 'o' || source[marker + 2U] != 'd' || source[marker + 3U] != 'i' || source[marker + 4U] != 'n' || source[marker + 5U] != 'g') continue;
            name_start = marker + 6U;
            if (name_start == line_end || (source[name_start] != ':' && source[name_start] != '=')) continue;
            name_start += 1U;
            while (name_start < line_end && (source[name_start] == ' ' || source[name_start] == '\t')) name_start += 1U;
            name_end = name_start;
            while (name_end < line_end && ((source[name_end] >= 'a' && source[name_end] <= 'z') || (source[name_end] >= 'A' && source[name_end] <= 'Z') || (source[name_end] >= '0' && source[name_end] <= '9') || source[name_end] == '-' || source[name_end] == '_' || source[name_end] == '.')) name_end += 1U;
        ...
        position = line_end;
        if (position < size && source[position] == '\r') position += 1U;
...
static int32_t __tinypy_compiler_utf8_valid(const unsigned char *bytes, size_t size)
{
    ...
    while (index < size) {
        ...
        if ((length == 3U && code_point < 0x800U) || (length == 4U && code_point < 0x10000U) || code_point > 0x10ffffU || (code_point >= 0xd800U && code_point <= 0xdfffU)) return 0;
        index += length;
    }
    return 1;
}
...
void tinypy_internal_compiler_error(tinypy_compile_ctx_t *ctx, tinypy_error_kind_e error_kind, const char *message, int32_t line_number, int32_t column_offset, tinypy_error_t **out_error)
{
    ...
    if (line_number > 0 && ctx->source.bytes != NULL) {
        ...
        if (line == line_number) {
            ...
            if (end < ctx->source.size) line_size += 1U;
        }
    }
...
void tinypy_internal_compiler_error_parts(tinypy_compile_ctx_t *ctx, tinypy_error_kind_e error_kind, const char *const *parts, const size_t *part_sizes, size_t part_count, int32_t line_number, int32_t column_offset)
{
    ...
    for (index = 0U; index < part_count; index += 1U) {
        if (part_sizes[index] != 0U) (void)memcpy(message + offset, parts[index], part_sizes[index]);
...
int32_t tinypy_internal_compiler_source_prepare(tinypy_compile_ctx_t *ctx, const void *source, size_t source_size, tinypy_error_t **out_error)
{
    ...
    if (__tinypy_compiler_encoding_cookie(input + input_offset, source_size - input_offset, &cookie, &cookie_size, &cookie_line) != 0) {
        ...
        if (__tinypy_compiler_is_latin1_cookie(cookie, cookie_size) != 0) latin1 = 1;
        else if (__tinypy_compiler_is_ascii_cookie(cookie, cookie_size) != 0) ascii = 1;
    ...
    assert(source_size <= (SIZE_MAX - 2U) / 2U);
    output_capacity = latin1 != 0 ? (source_size - input_offset) * 2U + 2U : source_size - input_offset + 2U;
    output = (unsigned char *)tinypy_internal_compiler_arena_allocate(ctx, output_capacity);
    if (output == NULL) {
    ...
    for (input_index = input_offset; input_index < source_size; input_index += 1U) {
        ...
        if (byte == '\r') {
            ...
            output[output_size] = (unsigned char)(0xc0U | (byte >> 6U));
            output[output_size + 1U] = (unsigned char)(0x80U | (byte & 0x3fU));
            output_size += 2U;
        } else {
