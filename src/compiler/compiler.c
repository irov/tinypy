...
static uint32_t __tinypy_compiler_inherited_flags(const tinypy_compile_ctx_t *ctx)
{
    ...
    if (ctx->options.dont_inherit != 0) return ctx->options.flags;
    frame = tinypy_vm_current_frame(ctx->vm);
    if (frame == NULL) return ctx->options.flags;
    return ctx->options.flags | ((uint32_t)tinypy_code_flags(tinypy_frame_code(frame)) & future_mask);
}
...
{
    if (mode == TINYPY_COMPILE_EXEC) return TINYPY_GRAMMAR_FILE_INPUT;
    if (mode == TINYPY_COMPILE_EVAL) return TINYPY_GRAMMAR_EVAL_INPUT;
...
static int __tinypy_compiler_parser_flags(uint32_t compile_flags)
{
    ...
    if ((compile_flags & (uint32_t)TINYPY_CODE_FUTURE_PRINT_FUNCTION) != 0U) flags |= TINYPY_PARSER_FLAG_PRINT_IS_FUNCTION;
    if ((compile_flags & (uint32_t)TINYPY_CODE_FUTURE_UNICODE_LITERALS) != 0U) flags |= TINYPY_PARSER_FLAG_UNICODE_LITERALS;
    if ((compile_flags & (uint32_t)TINYPY_COMPILE_FLAG_DONT_IMPLY_DEDENT) != 0U) flags |= TINYPY_PARSER_FLAG_DONT_IMPLY_DEDENT;
    return flags;
}

static tinypy_error_kind_e __tinypy_compiler_parser_error_kind(int error, int token, int expected)
{
    if (error == TINYPY_PARSER_TAB_SPACE_ERROR) return TINYPY_ERROR_TAB;
    if (error == TINYPY_PARSER_DEDENT_ERROR || error == TINYPY_PARSER_TOO_DEEP) return TINYPY_ERROR_INDENTATION;
    if (error == TINYPY_PARSER_SYNTAX_ERROR && (token == TINYPY_TOKEN_INDENT || expected == TINYPY_TOKEN_INDENT)) return TINYPY_ERROR_INDENTATION;
    if (error == TINYPY_PARSER_DECODE_ERROR) return TINYPY_ERROR_SOURCE_DECODING;
    if (error == TINYPY_PARSER_OUT_OF_MEMORY || error == TINYPY_PARSER_OVERFLOW) return TINYPY_ERROR_COMPILER_LIMIT;
    return TINYPY_ERROR_SYNTAX;
}

static const char *__tinypy_compiler_parser_error_message(int error, int token, int expected)
{
    if (error == TINYPY_PARSER_EOF) return "unexpected EOF while parsing";
    if (error == TINYPY_PARSER_BAD_TOKEN) return "invalid token";
    if (error == TINYPY_PARSER_TAB_SPACE_ERROR) return "inconsistent use of tabs and spaces in indentation";
    if (error == TINYPY_PARSER_TOO_DEEP) return "too many levels of indentation";
    if (error == TINYPY_PARSER_DEDENT_ERROR) return "unindent does not match any outer indentation level";
    if (error == TINYPY_PARSER_DECODE_ERROR) return "source decoding failed";
    if (error == TINYPY_PARSER_EOF_TRIPLE_STRING) return "EOF while scanning triple-quoted string literal";
    if (error == TINYPY_PARSER_EOL_STRING) return "EOL while scanning string literal";
    if (error == TINYPY_PARSER_LINE_CONTINUATION_ERROR) return "unexpected character after line continuation character";
    if (error == TINYPY_PARSER_OUT_OF_MEMORY || error == TINYPY_PARSER_OVERFLOW) return "parser exceeds compiler limits";
    if (error == TINYPY_PARSER_SYNTAX_ERROR && token == TINYPY_TOKEN_INDENT) return "unexpected indent";
    if (error == TINYPY_PARSER_SYNTAX_ERROR && expected == TINYPY_TOKEN_INDENT) return "expected an indented block";
...
static tinypy_cst_node_t *__tinypy_compiler_parse(tinypy_compile_ctx_t *ctx, tinypy_error_t **out_error)
{
    ...
    int flags = __tinypy_compiler_parser_flags(__tinypy_compiler_inherited_flags(ctx));
    tinypy_cst_node_t *tree;

    tree = tinypy_internal_parse_source(ctx, (const char *)ctx->source.bytes, ctx->source.size, ctx->logical_filename, &__tinypy_parser_grammar, __tinypy_compiler_parser_start(ctx->options.mode), &detail, &flags);
    if (tree != NULL) return tree;
    tinypy_internal_compiler_error(ctx, __tinypy_compiler_parser_error_kind(detail.result, detail.token, detail.expected), __tinypy_compiler_parser_error_message(detail.result, detail.token, detail.expected), detail.line_number > 0 ? detail.line_number : 1, detail.offset >= 0 ? detail.offset : 1, out_error);
    return NULL;
}
...
tinypy_value_t *tinypy_internal_compiler_compile(tinypy_compile_ctx_t *ctx, tinypy_error_t **out_error)
{
    ...
    if (ctx->options.mode == TINYPY_COMPILE_EXEC && __tinypy_compiler_source_is_empty_suite(ctx) != 0) return __tinypy_compiler_empty_code(ctx);
    if (ctx->options.mode == TINYPY_COMPILE_SINGLE && __tinypy_compiler_source_is_empty_suite(ctx) != 0 && ctx->source.size > 1U) return __tinypy_compiler_empty_code(ctx);
    ...
    if (code == NULL && ctx->failed == 0) tinypy_internal_compiler_error(ctx, TINYPY_ERROR_COMPILER_LIMIT, "code generation exceeds compiler limits", 1, 1, out_error);
