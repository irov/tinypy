...
tinypy_cst_node_t *
tinypy_internal_compiler_cst_new(tinypy_compile_ctx_t *ctx, int type)
{
 ...
 */
#define TINYPY_CST_ROUND_UP(n) ((n) <= 1 ? (n) :                 \
               (n) <= 128 ? (((n) + 3) & ~3) :          \
               __fancy_roundup(n))


int
tinypy_internal_compiler_cst_add_child(register tinypy_cst_node_t *n1, int type, char *str, int lineno, int col_offset)
{
...
void
tinypy_internal_compiler_cst_release(tinypy_cst_node_t *n)
{
    (void)n;
}

tinypy_compiler_size_t
tinypy_internal_compiler_cst_size(tinypy_cst_node_t *n)
{
...
static tinypy_compiler_size_t
__tinypy_frontend_size_of_children(tinypy_cst_node_t *n)
{
    tinypy_compiler_size_t res = 0;
    int i;
