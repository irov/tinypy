    ...
    */
    ...
    reoptimize_current:
        ...
        case TINYPY_OP_UNARY_NOT:
            if (codestr[i + 1] != TINYPY_OP_POP_JUMP_IF_FALSE || !TINYPY_OPTIMIZER_IS_BASIC_BLOCK(blocks, i, 4)) {
                continue;
            }
        ...
        case TINYPY_OP_COMPARE_OP:
            ...
            if (j < 6 || j > 9 || codestr[i + 3] != TINYPY_OP_UNARY_NOT || !TINYPY_OPTIMIZER_IS_BASIC_BLOCK(blocks, i, 4)) {
                continue;
            }
        ...
        case TINYPY_OP_LOAD_CONST:
            cumlc = lastlc + 1;
            j = TINYPY_OPTIMIZER_GET_ARGUMENT(codestr, i);
            if (codestr[i + 3] != TINYPY_OP_POP_JUMP_IF_FALSE || !TINYPY_OPTIMIZER_IS_BASIC_BLOCK(blocks, i, 6) || !TINYPY_COMPILER_OBJECT_IS_TRUE(TINYPY_COMPILER_LIST_GET_ITEM(consts, j))) {
                continue;
            }
        ...
        case TINYPY_OP_BUILD_LIST:
            ...
            if (codestr[i + 3] != TINYPY_OP_UNPACK_SEQUENCE || !TINYPY_OPTIMIZER_IS_BASIC_BLOCK(blocks, i, 6) || j != TINYPY_OPTIMIZER_GET_ARGUMENT(codestr, i + 3)) {
                continue;
            }
            if (j == 1) {
                memset(codestr + i, TINYPY_OP_NOP, 6);
        ...
        case TINYPY_OP_RETURN_VALUE:
            ...
            if (codestr[i + 4] == TINYPY_OP_RETURN_VALUE && TINYPY_OPTIMIZER_IS_BASIC_BLOCK(blocks, i, 5)) {
                memset(codestr + i + 1, TINYPY_OP_NOP, 4);
            }
            else if (TINYPY_OPTIMIZER_IS_UNCONDITIONAL_JUMP(codestr[i + 1]) && TINYPY_OPTIMIZER_IS_BASIC_BLOCK(blocks, i, 4)) {
