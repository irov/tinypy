...
static const char *const __tinypy_unicode_name_hangul_syllables[TINYPY_UNICODE_NAME_T_COUNT][3] = {
    {"G", "A", ""},
    {"GG", "AE", "G"},
    {"N", "YA", "GG"},
    {"D", "YAE", "GS"},
...
static uint32_t __tinypy_unicode_name_hash(const char *name, size_t name_size)
{
    ...
    for (index = 0U; index < name_size; index += 1U) {
        ...
        if (overflow != 0U) hash = (hash ^ ((overflow >> 24U) & UINT32_C(0xff))) & UINT32_C(0x00ffffff);
    }
    return hash;
}
...
{
    return (code_point >= UINT32_C(0x3400) && code_point <= UINT32_C(0x4db5)) ||
        (code_point >= UINT32_C(0x4e00) && code_point <= UINT32_C(0x9fcb)) ||
        (code_point >= UINT32_C(0x20000) && code_point <= UINT32_C(0x2a6d6)) ||
        (code_point >= UINT32_C(0x2a700) && code_point <= UINT32_C(0x2b734));
}
...
static int32_t __tinypy_unicode_name_append(char *buffer, size_t capacity, size_t *size, const char *text)
{
    ...
    if (text_size > capacity - *size) return INT32_C(0);
...
static int32_t __tinypy_unicode_name_code_to_name(uint32_t code_point, char *buffer, size_t capacity)
{
    ...
    if (code_point >= UINT32_C(0x110000) || capacity == 0U) return INT32_C(0);
    if (code_point >= TINYPY_UNICODE_NAME_S_BASE && code_point < TINYPY_UNICODE_NAME_S_BASE + TINYPY_UNICODE_NAME_S_COUNT) {
        ...
        if (__tinypy_unicode_name_append(buffer, capacity, &size, "HANGUL SYLLABLE ") == 0 ||
            __tinypy_unicode_name_append(buffer, capacity, &size, __tinypy_unicode_name_hangul_syllables[leading][0]) == 0 ||
            __tinypy_unicode_name_append(buffer, capacity, &size, __tinypy_unicode_name_hangul_syllables[vowel][1]) == 0 ||
            __tinypy_unicode_name_append(buffer, capacity, &size, __tinypy_unicode_name_hangul_syllables[trailing][2]) == 0 || size == capacity) return INT32_C(0);
        buffer[size] = '\0';
        return INT32_C(1);
    }
    if (__tinypy_unicode_name_is_unified_ideograph(code_point) != 0) {
        ...
        if (__tinypy_unicode_name_append(buffer, capacity, &size, prefix) == 0) return INT32_C(0);
        do {
        ...
        if (digit_count >= capacity - size) return INT32_C(0);
        while (digit_count != 0U) buffer[size++] = reversed[--digit_count];
        buffer[size] = '\0';
        return INT32_C(1);
    }
    {
        ...
        for (;;) {
            ...
            if (output_started != 0) {
                if (size >= capacity) return INT32_C(0);
                buffer[size++] = ' ';
            }
...
static int32_t __tinypy_unicode_name_equal(uint32_t code_point, const char *name, size_t name_size)
{
    ...
    if (name_size >= sizeof(buffer) || __tinypy_unicode_name_code_to_name(code_point, buffer, sizeof(buffer)) == 0) return INT32_C(0);
    for (index = 0U; index < name_size; index += 1U) {
        if (__tinypy_unicode_name_upper((unsigned char)name[index]) != (unsigned char)buffer[index]) return INT32_C(0);
    }
...
static void __tinypy_unicode_name_find_syllable(const char *text, size_t text_size, size_t count, size_t column, size_t *out_length, int32_t *out_position)
{
    ...
    *out_position = -1;
    for (index = 0U; index < count; index += 1U) {
        ...
        if ((*out_position >= 0 && syllable_size <= *out_length) || syllable_size > text_size || memcmp(text, syllable, syllable_size) != 0) continue;
...
int32_t tinypy_internal_compiler_unicode_name(const char *name, size_t name_size, uint32_t *out_code_point)
{
    ...
    if (name_size >= sizeof(hangul_prefix) - 1U && memcmp(name, hangul_prefix, sizeof(hangul_prefix) - 1U) == 0) {
        ...
        position += length;
        remaining -= length;
        if (leading < 0 || vowel < 0 || trailing < 0 || remaining != 0U) return INT32_C(0);
    ...
    if (name_size >= sizeof(ideograph_prefix) - 1U && memcmp(name, ideograph_prefix, sizeof(ideograph_prefix) - 1U) == 0) {
        ...
        if (name_size - position != 4U && name_size - position != 5U) return INT32_C(0);
        code_point = 0U;
        ...
        if (__tinypy_unicode_name_is_unified_ideograph(code_point) == 0) return INT32_C(0);
        *out_code_point = code_point;
        return INT32_C(1);
    }
    if (name_size == 0U || name_size >= NAME_MAXLEN) return INT32_C(0);
    hash = __tinypy_unicode_name_hash(name, name_size);
    index = (~hash) & mask;
    code_point = code_hash[index];
    if (code_point == 0U) return INT32_C(0);
    if (__tinypy_unicode_name_equal(code_point, name, name_size) != 0) {
        *out_code_point = code_point;
        return INT32_C(1);
    }
    increment = (hash ^ (hash >> 3U)) & mask;
    if (increment == 0U) increment = mask;
    for (;;) {
        ...
        increment <<= 1U;
        if (increment > mask) increment ^= (uint32_t)code_poly;
    }
}
