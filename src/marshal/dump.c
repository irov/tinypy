...
{
    if (writer->result != TINYPY_MARSHAL_OK) return;
...
static int32_t __tinypy_marshal_dump_put(tinypy_marshal_dump_writer_t *writer, const void *bytes, size_t size)
{
    ...
    if (writer->buffer != NULL) {
        ...
        if (size != 0U) (void)memcpy(writer->buffer + writer->offset, bytes, size);
    }
    writer->offset += size;
    return INT32_C(1);
}
...
static int32_t __tinypy_marshal_dump_i64(tinypy_marshal_dump_writer_t *writer, int64_t value)
{
    ...
    for (index = 0U; index < sizeof(bytes); index += 1U) bytes[index] = (unsigned char)(bits >> (index * 8U));
    return __tinypy_marshal_dump_put(writer, bytes, sizeof(bytes));
}
...
static ptrdiff_t __tinypy_marshal_dump_find_intern(const tinypy_marshal_dump_writer_t *writer, const unsigned char *bytes, size_t size)
{
    ...
    for (index = 0U; index < writer->intern_count; index += 1U) {
        size_t candidate_size;
        const unsigned char *candidate = (const unsigned char *)tinypy_string_view(writer->interns[index], &candidate_size);

        if (candidate_size == size && (size == 0U || memcmp(candidate, bytes, size) == 0)) return (ptrdiff_t)index;
    }
    return -1;
}
...
static void __tinypy_marshal_dump_add_intern(tinypy_marshal_dump_writer_t *writer, const tinypy_value_t *value)
{
    if (writer->intern_count == writer->intern_capacity) {
        ...
        if (writer->interns == NULL) writer->interns = (const tinypy_value_t **)tinypy_internal_vm_allocate(writer->vm, new_size, (uint32_t)TINYPY_ALLOC_TAG_MARSHAL_WRITE);
        else writer->interns = (const tinypy_value_t **)tinypy_internal_vm_reallocate(writer->vm, (void *)writer->interns, old_size, new_size, (uint32_t)TINYPY_ALLOC_TAG_MARSHAL_WRITE);
...
static int32_t __tinypy_marshal_dump_string(tinypy_marshal_dump_writer_t *writer, const tinypy_value_t *value, tinypy_marshal_dump_string_context_e context)
{
    ...
    if (interned != 0) {
        ...
        if (index >= 0) return __tinypy_marshal_dump_u8(writer, (uint8_t)'R') && __tinypy_marshal_dump_i32(writer, (int32_t)index);
        __tinypy_marshal_dump_add_intern(writer, value);
        if (__tinypy_marshal_dump_u8(writer, (uint8_t)'t') == 0) return INT32_C(0);
    } else if (__tinypy_marshal_dump_u8(writer, (uint8_t)'s') == 0) return INT32_C(0);
...
static int32_t __tinypy_marshal_dump_tuple(tinypy_marshal_dump_writer_t *writer, const tinypy_value_t *tuple, size_t depth, tinypy_marshal_dump_string_context_e item_context)
{
    ...
    if (__tinypy_marshal_dump_u8(writer, (uint8_t)'(') == 0 || __tinypy_marshal_dump_size32(writer, count) == 0) return INT32_C(0);
    for (index = 0U; index < count; index += 1U) if (__tinypy_marshal_dump_value(writer, tinypy_tuple_get(tuple, index), depth + 1U, item_context) == 0) return INT32_C(0);
    return INT32_C(1);
}
...
static int32_t __tinypy_marshal_dump_code(tinypy_marshal_dump_writer_t *writer, const tinypy_value_t *value, size_t depth)
{
    ...
    return __tinypy_marshal_dump_u8(writer, (uint8_t)'c') &&
        __tinypy_marshal_dump_i32(writer, code->arg_count) &&
        __tinypy_marshal_dump_i32(writer, code->local_count) &&
        __tinypy_marshal_dump_i32(writer, code->stack_size) &&
        __tinypy_marshal_dump_i32(writer, code->flags) &&
        __tinypy_marshal_dump_value(writer, code->bytecode, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_NORMAL) &&
        __tinypy_marshal_dump_tuple(writer, code->consts, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_LITERAL) &&
        __tinypy_marshal_dump_tuple(writer, code->names, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_IDENTIFIER) &&
        __tinypy_marshal_dump_tuple(writer, code->varnames, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_IDENTIFIER) &&
        __tinypy_marshal_dump_tuple(writer, code->freevars, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_IDENTIFIER) &&
        __tinypy_marshal_dump_tuple(writer, code->cellvars, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_IDENTIFIER) &&
        __tinypy_marshal_dump_value(writer, code->filename, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_NORMAL) &&
        __tinypy_marshal_dump_value(writer, code->name, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_CODE_NAME) &&
        __tinypy_marshal_dump_i32(writer, code->first_line_number) &&
        __tinypy_marshal_dump_value(writer, code->lnotab, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_NORMAL);
}

static int32_t __tinypy_marshal_dump_set(tinypy_marshal_dump_writer_t *writer, const tinypy_value_t *value, size_t depth)
{
    const tinypy_dict_object_t *dict = TINYPY_DICT_OBJECT(TINYPY_SET_OBJECT((tinypy_value_t *)value)->dict);
    size_t index;

    if (__tinypy_marshal_dump_u8(writer, tinypy_typeof(value) == TINYPY_VALUE_FROZENSET ? (uint8_t)'>' : (uint8_t)'<') == 0 || __tinypy_marshal_dump_size32(writer, dict->used) == 0) return INT32_C(0);
    for (index = 0U; index <= dict->mask; index += 1U) {
        const tinypy_dict_entry_t *entry = &dict->table[index];

        if (entry->state == TINYPY_DICT_ENTRY_ACTIVE && __tinypy_marshal_dump_value(writer, entry->key, depth + 1U, TINYPY_MARSHAL_DUMP_STRING_LITERAL) == 0) return INT32_C(0);
    }
    return INT32_C(1);
}
...
static int32_t __tinypy_marshal_dump_value(tinypy_marshal_dump_writer_t *writer, const tinypy_value_t *value, size_t depth, tinypy_marshal_dump_string_context_e string_context)
{
    ...
    switch (type) {
    case TINYPY_VALUE_NONE: return __tinypy_marshal_dump_u8(writer, (uint8_t)'N');
    case TINYPY_VALUE_BOOL: return __tinypy_marshal_dump_u8(writer, tinypy_bool_as_i32(value) != 0 ? (uint8_t)'T' : (uint8_t)'F');
    case TINYPY_VALUE_ELLIPSIS: return __tinypy_marshal_dump_u8(writer, (uint8_t)'.');
    case TINYPY_VALUE_INTEGER: {
        int64_t integer = tinypy_integer_as_i64(value);

        if (integer >= INT32_MIN && integer <= INT32_MAX) return __tinypy_marshal_dump_u8(writer, (uint8_t)'i') && __tinypy_marshal_dump_i32(writer, (int32_t)integer);
    ...
    case TINYPY_VALUE_LONG: {
        ...
        if (__tinypy_marshal_dump_u8(writer, (uint8_t)'l') == 0 || __tinypy_marshal_dump_i32(writer, (int32_t)count * sign) == 0) return INT32_C(0);
        for (index = 0U; index < count; index += 1U) if (__tinypy_marshal_dump_u16(writer, digits[index]) == 0) return INT32_C(0);
        return INT32_C(1);
    }
    case TINYPY_VALUE_FLOAT: return __tinypy_marshal_dump_u8(writer, (uint8_t)'g') && __tinypy_marshal_dump_double(writer, tinypy_float_as_double(value));
    ...
    case TINYPY_VALUE_STRING: return __tinypy_marshal_dump_string(writer, value, string_context);
    ...
    case TINYPY_VALUE_TUPLE: return __tinypy_marshal_dump_tuple(writer, value, depth, string_context);
    case TINYPY_VALUE_CODE: return __tinypy_marshal_dump_code(writer, value, depth);
    case TINYPY_VALUE_SET:
    case TINYPY_VALUE_FROZENSET: return __tinypy_marshal_dump_set(writer, value, depth);
    default:
...
{
    if (writer->interns != NULL) tinypy_internal_vm_deallocate(writer->vm, (void *)writer->interns, writer->intern_capacity * sizeof(*writer->interns), (uint32_t)TINYPY_ALLOC_TAG_MARSHAL_WRITE);
}
...
static tinypy_marshal_result_e __tinypy_marshal_dump_run(const tinypy_value_t *code, void *buffer, size_t capacity, const tinypy_marshal_write_options_t *options, size_t *out_size, tinypy_marshal_error_t *out_error)
{
    ...
    writer.vm = tinypy_internal_value_vm(code);
...
tinypy_marshal_result_e tinypy_marshal_dump_code_v2(const tinypy_value_t *code, void *buffer, size_t capacity, size_t *out_size, const tinypy_marshal_write_options_t *options, tinypy_marshal_error_t *out_error)
{
    ...
    if (out_error != NULL) (void)memset(out_error, 0, sizeof(*out_error));
    result = __tinypy_marshal_dump_run(code, NULL, 0U, options, &required_size, out_error);
    *out_size = required_size;
    if (result != TINYPY_MARSHAL_OK) return result;
    if (buffer == NULL) return TINYPY_MARSHAL_OK;
