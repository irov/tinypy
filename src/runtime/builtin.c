    ...
    if (callable->type == &vm->types[TINYPY_VALUE_METHOD]) {
        ...
        if (method->self != NULL && method->function->type == &vm->types[TINYPY_VALUE_FUNCTION] && item_count < SIZE_MAX / sizeof(tinypy_value_t *)) {
            ...
            if (bound_count > sizeof(local_items) / sizeof(local_items[0])) {
                bound_items = (tinypy_value_t **)tinypy_internal_vm_allocate(vm, bound_count * sizeof(*bound_items));
            }
...
static tinypy_value_t *__tinypy_builtin_len(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    if (tinypy_internal_object_has_special_override(value, "__len__", 7U) != 0) {
        tinypy_value_t *method = tinypy_object_get_attr(value, "__len__", 7U, out_error);
...
static tinypy_value_t *__tinypy_builtin_enumerate(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    tinypy_vm_t *vm = TINYPY_VALUE_VM(function);
    static const char *const parameter_names[] = {"sequence", "start"};
    static const size_t parameter_name_sizes[] = {8U, 5U};
    tinypy_value_t *arguments[2];
    tinypy_value_t *counter;
    tinypy_value_t *result;
    ...
    if (arguments[1] == NULL) {
        counter = tinypy_integer_from_i64(vm, INT64_C(0));
    }
    else {
        counter = tinypy_internal_index_value(arguments[1], out_error);
        if (counter == NULL) {
            return NULL;
        }
    }
    result = tinypy_internal_enumerate_new(arguments[0], counter, out_error);
...
static tinypy_value_t *__tinypy_builtin_filter_text(tinypy_vm_t *vm, tinypy_value_t *predicate, tinypy_value_t *input, tinypy_error_t **out_error) {
    ...
    if (TINYPY_VALUE_KIND(predicate) == TINYPY_VALUE_NONE) {
        tinypy_value_t *return_value_1 = unicode != 0
            ? tinypy_unicode_from_utf8(vm, (const char *)bytes, byte_size)
            : tinypy_string_from_bytes(vm, bytes, byte_size);
        return return_value_1;
    }

    uint8_t *output = byte_size != 0U ? (uint8_t *)tinypy_internal_vm_allocate(vm, byte_size) : NULL;
    ...
    tinypy_value_t *result = unicode != 0
        ? tinypy_unicode_from_utf8(vm, (const char *)output, output_size)
        : tinypy_string_from_bytes(vm, output, output_size);
    if (output != NULL) {
        tinypy_internal_vm_deallocate(vm, output, byte_size);
    }
    return result;
...
static tinypy_value_t *__tinypy_builtin_filter(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    tinypy_internal_list_reserve(vm, selected, tinypy_internal_iterable_size_hint(input));
    for (;;) {
    ...
    if (TINYPY_VALUE_KIND(input) == TINYPY_VALUE_TUPLE) {
        ...
        if (total != 0U) {
            buffer = (uint8_t *)tinypy_internal_vm_allocate(vm, total);
        }
        iterator = TINYPY_LIST_ITERATOR_BEGIN(selected);
        iterator_end = TINYPY_LIST_ITERATOR_END(selected);
        for (; iterator != iterator_end; ++iterator) {
        ...
        result = TINYPY_VALUE_KIND(input) == TINYPY_VALUE_UNICODE ? tinypy_unicode_from_utf8(vm, (const char *)buffer, total) : tinypy_string_from_bytes(vm, buffer, total);
        if (total != 0U) {
...
static tinypy_value_t *__tinypy_builtin_map(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    tinypy_value_t **iterators = (tinypy_value_t **)tinypy_internal_vm_allocate(vm, iterable_count * sizeof(*iterators));
    tinypy_value_t **call_items = (tinypy_value_t **)tinypy_internal_vm_allocate(vm, iterable_count * sizeof(*call_items));
    uint8_t *exhausted = (uint8_t *)tinypy_internal_vm_allocate(vm, iterable_count * sizeof(*exhausted));
    uint8_t *yielded = (uint8_t *)tinypy_internal_vm_allocate(vm, iterable_count * sizeof(*yielded));
    ...
    for (iterable_index = 0U; iterable_index < iterable_count; ++iterable_index) {
        ...
        if (iterators[iterable_index] == NULL) {
            ...
            tinypy_internal_vm_deallocate(vm, yielded, iterable_count * sizeof(*yielded));
            tinypy_internal_vm_deallocate(vm, exhausted, iterable_count * sizeof(*exhausted));
            tinypy_internal_vm_deallocate(vm, call_items, iterable_count * sizeof(*call_items));
            tinypy_internal_vm_deallocate(vm, iterators, iterable_count * sizeof(*iterators));
            return NULL;
        }
    }
    result = tinypy_list_from_items(vm, NULL, 0U);
    tinypy_internal_list_reserve(vm, result, result_hint);
    for (;;) {
...
static tinypy_value_t *__tinypy_builtin_reversed(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    tinypy_value_t *item = TINYPY_TUPLE_GET(args, 0U);
    if (tinypy_internal_object_has_special(item, "__reversed__", 12U) != 0) {
        tinypy_value_t *method = tinypy_object_get_attr(item, "__reversed__", 12U, out_error);
        tinypy_value_t *empty;
        tinypy_value_t *result;
...
static tinypy_value_t *__tinypy_builtin_round(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    if (TINYPY_VALUE_KIND(value) == TINYPY_VALUE_FLOAT) {
        ...
        tinypy_value_t *method = tinypy_object_get_attr(value, "__float__", 9U, out_error);
...
static tinypy_value_t *__tinypy_builtin_base_repr(tinypy_value_t *function, tinypy_value_t *args, tinypy_value_t *kwargs, void *user_data, tinypy_error_t **out_error) {
    ...
    uint8_t *output = (uint8_t *)tinypy_internal_vm_allocate(vm, output_size);
    ...
    tinypy_value_t *result = tinypy_string_from_bytes(vm, output, output_index);
    tinypy_internal_vm_deallocate(vm, output, output_size);
